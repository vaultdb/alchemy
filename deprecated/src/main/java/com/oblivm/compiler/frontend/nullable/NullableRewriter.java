...
public class NullableRewriter extends DefaultStatementExpressionVisitor<ASTStatement, Pair<ASTExpression, List<ASTType>>> {
	...
	@Override
	public Pair<ASTExpression, List<ASTType>> visit(ASTStringConstantExpression constantExpression) {
		return new Pair<ASTExpression, List<ASTType>>(constantExpression, one(ASTStringType.get(32, ASTLabel.Pub)));
	...
	public ASTStatement visit(ASTSetNonNullArrayEntriesStatement stmt) {
		stmt.expr = visit(stmt.expr).left;
		return stmt;
	}

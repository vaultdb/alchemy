...
import com.oblivm.compiler.ast.stmt.ASTSetNonNullArrayEntriesStatement;
import com.oblivm.compiler.ast.stmt.ASTFuncStatement;
import com.oblivm.compiler.ast.stmt.ASTGetNonNullArrayEntriesStatement;
...
import com.oblivm.compiler.type.manage.StringType;
...
//			input.addAll(tmp.left);
	...
	public IRCode visit(ASTSetNonNullArrayEntriesStatement stmt) {
		...
		Pair<List<Variable>, IRCode> expr = visit(stmt.expr);

		Variable lv = var.left.get(0);
		Variable rv = expr.left.get(0);
		IRCode code = expr.right;

		
		return new SetNonNullArrayEntries(code, lv, rv);
	}


	@Override
	public IRCode visit(ASTGetNonNullArrayEntriesStatement stmt) {
		Pair<List<Variable>, IRCode> var = visit(stmt.var);
		Pair<List<Variable>, IRCode> expr = visit(stmt.expr);
		Variable lv = var.left.get(0);
		Variable rv = expr.left.get(0);
		...
		return new GetNonNullArrayEntries(code, lv, rv);
	}

	@Override
	public IRCode visit(ASTWriteArrayStatement stmt) {
		Pair<List<Variable>, IRCode> code = visit(stmt.array);
		Variable arr = code.left.get(0);
		
		Pair<List<Variable>, IRCode>  fname = visit(stmt.dstFile);
		Assign a = (Assign) fname.right;
		ConstExp c = (ConstExp) a.exp;
		
		return new WriteArray(arr, c);
	}

	@Override
	public IRCode visit(ASTReadArrayStatement stmt) {
		Pair<List<Variable>, IRCode> code = visit(stmt.array);
		Variable arr = code.left.get(0);
		
		Pair<List<Variable>, IRCode>  fname = visit(stmt.srcFile);
		Assign a = (Assign) fname.right;
		ConstExp c = (ConstExp) a.exp;

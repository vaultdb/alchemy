...
public:
  ...
  Data *SeqScan0Union(int party, NetIO *io) {
    ...
    Integer *res =
        new Integer[aliceSize + bobSize]; // enough space for all inputs
    Bit *tmp = new Bit[rowLength *
                       (aliceSize + bobSize)]; //  bit representation of res
    ...
    // reverse Alice's order for correct Bitonic Merge ( Increasing -> Max ->
    // Decreasing )
    for (int i = 0; i < aliceSize; ++i) {
      ...
      for (int j = 0; j < rowLength; ++j) {
        ...
        tmp[writeTuple * rowLength + j] =
            Bit((ALICE == party) ? *readPos : 0, ALICE);
    ...
    cout << "SeqScan0Union took as input " << aliceSize + bobSize
         << " tuples from alice and bob." << endl;
    cout << "Tuple length " << res[0].size() << endl;

    // TODO: make sort more robust.  Handle sort keys that are not adjacent or
    // in the same order in the table
    EmpUtilities::bitonicMergeSql(res, 0, aliceSize + bobSize, Bit(true), 0,
                                  273);
    ...
    for (int i = 0; i < aliceSize + bobSize; ++i) {
      ...
      // cout << "Outputting " << res[i][rowLength - 1].reveal(PUBLIC) << ", "
      // << value.reveal<int32_t>(PUBLIC) << endl;
    }
    return d;
  }

  /* @jennie This function needs to be generated*/

  std::unique_ptr<QueryTable> Aggregate4(QueryTable *t) {
    AggregateDef d;
  ...
  // expects as arguments party (1 = alice, 2 = bob) plus the port it will run
  // the protocols over

public:
  void setGeneratorHost(string host) { aliceHost = host; }

  void run(int party, int port) {

    std::cout << "starting run in emp! party=" << party << " port=" << port
              << std::endl;

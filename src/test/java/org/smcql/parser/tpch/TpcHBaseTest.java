...
public class TpcHBaseTest extends BaseTest {
	...
	protected static final List<String> QUERIES = ImmutableList.of(
			...
			"with lis as (SELECT DISTINCT l_orderkey FROM lineitem WHERE l_commitdate < l_receiptdate) \n" +
					"\n" +
					"select\n" +
					"  o_orderpriority,\n" +
		...
		// TODO: address handling date intervals
		      ...
		      // 13
		          ...
		          + "     SELECT o_custkey, o_orderkey, o.o_comment not like '%special%requests%' not_special\n"
		          + "     FROM orders o)\n"
		          + " select\n"
		          + "  c_count,\n"
		          + "  count(*) as custdist\n"
		          ...
		          + "    from\n"
		          + "      customer c \n"
		          + "      left outer join tagged_orders o \n"
		          + "        on c.c_custkey = o.o_custkey and  not_special\n"
		          + "    group by\n"
		          + "      c.c_custkey\n"

...
import org.vaultdb.plan.ShadowRelNode;
...
public abstract class Operator implements CodeGenerator {
  ...
  public Operator(String name, SecureRelNode src, Operator... childOps) throws Exception {
    ...
    pCost =
        new PrivacyCost(
            epsilon, delta); // Epsilon Cost = Epsilon value * # relations touched (1 by default)
  }
  ...
  boolean sliceAble() {
    if (this.sliceAgnostic == true && !this.maxChildMode().oblivious) return true;
    return false;
  }
  ...
  public String toString() {
    ...
    String ret =
        baseRelNode.getRelNode().getRelTypeName() + "-" + executionMode + ", schema:" + getSchema();
    List<SecureRelDataTypeField> sliceAttrs = this.getSliceAttributes();
    if (!sliceAttrs.isEmpty()) ret += ", slice key: " + sliceAttrs;
  ...
  public String getPackageName() {
    ...
    return "org.vaultdb.generated." + queryName + "." + opName + operatorId;
  }
  ...
  // if a SMC operator implementation leverages the order of tuples for comparisons, codify that in
  // this function
  ...
  public String destFilename(ExecutionMode e) {
    if (!e.distributed || (e.distributed && !e.oblivious)) {
      return Utilities.getCodeGenTarget()
          + "/"
          + getQueryId()
          + "/sql/"
          + getOperatorId()
  ...
  public boolean sharesExecutionProperties(Operator op) {
    if (executionMode != op.getExecutionMode()) return false;

    if (executionMode.sliced
        && executionMode.oblivious
        && op.getExecutionMode() == this.executionMode) {
  ...
  public void addSqlGenerationNode(RelNode project) {
    ...
    if (!children.isEmpty()) children.get(0).addSqlGenerationNode(next);
  }
  ...
  // TODO: Implement code to estimate cardinality of output (true cardinality plus number pulled
  // from LaPlace distribution (which depends on the sensitivity of f aka the operator)
  ...
  public int getEstimatedSensitivity() {
    ...
    for (Operator child : children) {
      ...
      if (cSensitivity > max) max = cSensitivity;
    }

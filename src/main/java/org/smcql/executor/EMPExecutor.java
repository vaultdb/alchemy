...
public class EMPExecutor implements Runnable {
  ...
  public EMPExecutor(QueryCompiler qc, List<String> parties) throws Exception {
    ...
    if (remotePath == null) remotePath = "/tmp/smcql";

    prepareWorkers(workers);
    compiledPlan = qc;
    queryId = compiledPlan.getQueryId();
    empCode = compiledPlan.getEmpCode();
  }
  ...
  private void prepareWorkers(List<WorkerConfiguration> workers) throws Exception {
    ...
    cloud.node("**").setProp("smcql.empCode", empCode);
    cloud.node("**").setProp("smcql.queryId", queryId);
    cloud.node("**").touch();
    cloud
        .node("**")
        .massExec(
  ...
  public static String getRemotePathToJar(final String partOfJarName) throws Exception {
    for (Classpath.ClasspathEntry cpe :
        Classpath.getClasspath(ClassLoader.getSystemClassLoader())) {
            ...
            .massExec(
                new Callable<boolean[]>() {
                  ...
                  public boolean[] call() throws Exception {
                    ...
                    String outputFile = Utilities.getCodeGenTarget() + System.getProperty("smcql.empCode") + ".h" ;
                    Utilities.writeFile(outputFile,System.getProperty("smcql.empCode"));
                    EmpCompiler compiler = new EmpCompiler(compiledPlan.getPlan().getName());
                    compiler.compile();
                    //EmpProgram runner =
                     //   EmpCompiler.loadClass(compiledPlan.getPlan().getName(), party, 54321);
                    //runner.runProgram();

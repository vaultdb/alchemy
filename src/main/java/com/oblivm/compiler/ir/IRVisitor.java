...
public abstract class IRVisitor<T1, T2> {
	...
	public T1 visit(IRCode ast) {
		...
		} else if(ast instanceof SetNonNullArrayEntries) {
			return visit((SetNonNullArrayEntries)ast);
		} else if(ast instanceof GetNonNullArrayEntries) {
			return visit((GetNonNullArrayEntries)ast);

		} else if(ast instanceof WriteArray) {
			return visit((WriteArray)ast);
		} 
		else if(ast instanceof ReadArray) {
			return visit((ReadArray)ast);
		}else
			throw new RuntimeException("Unknown SCVM statement!" + ast.getClass());
	}
	...
	public abstract T1 visit(SetNonNullArrayEntries ret);

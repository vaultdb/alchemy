...
public class TypeEmittable extends Emittable {
	...
	public void emitConstructorInternal() {
		...
		out.println("\t\tthis.sortLib = new BitonicSortLib<"+codeGen.dataType+">(env);");
		out.println("\t\tthis.floatLib = new FloatLib<"+codeGen.dataType+">(env, 24, 8);");
		//out.println("\t\tretSize = null; // default case: means that input and output have same # of elements");
		if(!type.name.equals("NoClass")) {
	...
	public void emitReturnSize() {
		...
		out.println(codeGen.dataType+"[] getReturnSize() {");
	    out.println("\t\t return retSize;");
		out.println("}\n\n");
	}
	
	public void emitRun() {
		if(this.inheritWritable(type)) {
			//	 SecureArray<T> run(SecureArray<T> a, SecureArray<T> b) throws Exception;
			String arrayType = "SecureArray<" +codeGen.dataType + ">";
			out.println("public " + arrayType + " run(" + arrayType + " a, " + arrayType + " b) throws Exception {" );// placeholder for now
			out.println("      return null;");
			out.println("}\n\n");
		//TODO: put this where implemented main is now	
			//String arrayType = "SecureArray<" +codeGen.dataType + ">";
		//out.print(codeGen.dataType+"[] run(" + arrayType + " a, "+ arrayType + " b) throws Exception {\n");
	...
	public void emitAMethod(Method method) {
		...
		Iterator<Pair<Type, String> > paramItr = method.parameters.iterator();

		for(String s : method.bitParameters) {
			Pair<Type, String> var = paramItr.next();
			out.println("\t\t int "+s + " = (" + var.right + " != null) ? "+ var.right + ".length : 0;");
		
		}
		//out.println("\t\t int n = a.length;");
		//out.println("\t\t int m = b.length;");
	...
	private void emitMain() {
		...
		if (!(main.returnType instanceof IntType) && !(main.returnType instanceof ArrayType)) {
			throw new RuntimeException("Unsupported main function's return type: only support return int or array.");
		}
		...
		for(int i=0; i<inputs.length; ++i) {
			if (i < main.bitParameters.size()) {
				...
				if(type instanceof IntType) {
					...
					out.print("\t\t}\n");
					if(at.indexLab.lab == Label.Pub)
						inputs[i] = "com.oblivm.backend.lang.inter.Util.intToArray("+(j==0 ? "a" : "b")+", "+it.bit+", "+at.size+")";
					else
						inputs[i] = "com.oblivm.backend.lang.inter.Util.intToSecArray(env, "+(j==0 ? "a" : "b")+", "+it.bit+", "+at.size+")";
				} else {
		...
		if (main.returnType instanceof ArrayType) {
			out.print("\t\t"+codeGen.visit(main.returnType)+" output = run(");
		} else {
			out.print("\t\treturn run(");
		}
		

		int cnt = 0;
		for(int i=main.bitParameters.size(); i<inputs.length; ++i) {
			//System.out.println("Input " + i + " is " + inputs[i]);
			if(cnt > 0) out.print(", ");
			out.print(inputs[i]);
			++cnt;
		}

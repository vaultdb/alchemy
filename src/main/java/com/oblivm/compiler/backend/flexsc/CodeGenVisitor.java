...
public class CodeGenVisitor extends IRVisitor<String, Pair<String, String>> {
	...
	public Pair<String, String> visit(ArrayExp exp) {
		...
		if (type.indexLab.lab != Label.Pub) {
			...
			if(exp.idx.lab == Label.Pub) {
				...
				sb.append(this.visit(type.type)+" "+idx+" = env.inputOfAlice(Utils.fromLong("+exp.idx.name+", "+type.type.getBits()+"));\n");
			}
			sb.append(indent(indent));
	...
	public Pair<String, String> visit(BopExp exp) {
		...
		if(exp.getLabels() == Label.Pub) {
			sb.append(indent(indent));
			sb.append("long "+ret+" = "+exp.x1.name);
			...
			if ((exp.op == BopExp.Op.Shl || exp.op == BopExp.Op.Shr) && exp.x2.lab == Label.Pub) {
				...
				if(exp.x1.lab != Label.Pub) {
					...
					sb.append(indent(indent));
					if(exp.op == BopExp.Op.Shl)
						sb.append("long "+ret+" = "+exp.x1.name+" << "+exp.x2+";\n");
					else if(exp.op == BopExp.Op.Shr)
						sb.append("long "+ret+" = "+exp.x1.name+" >> "+exp.x2+";\n");
					else if(exp.op == BopExp.Op.Mul)
						sb.append("long "+ret+" = "+exp.x1.name+" * "+exp.x2+";\n");
				...
				if(exp.x1.lab == Label.Pub) {
					...
					if(exp.x1.type instanceof IntType)
						sb.append(dataType+"[] "+v1+" = env.inputOfAlice(Utils.fromLong("+exp.x1.name+", "+currentType.getBits()+"));\n");
					else
				...
				if(exp.x2.lab == Label.Pub) {
					...
					if(exp.x2.type instanceof IntType)
						sb.append(dataType+"[] "+v2+" = env.inputOfAlice(Utils.fromLong("+exp.x2.name+", "+currentType.getBits()+"));\n");
					else
...
//				sb.append("boolean "+ret+" = "+(exp.n != 0 ? true : false)+";\n");
				...
				ret = Long.toString(exp.n);
//				sb.append("int "+ret+" = "+exp.n+";\n");
		} else if(!exp.isString){ // long
			ret = Long.toString(exp.n);
//			sb.append("long "+ret+" = "+exp.v+";\n");
		}
		else { // isString
			ret = exp.s;
		}
		...
		if(exp.getLabels() != Label.Pub) {
			...
			if(exp.x2.lab == Label.Pub) {
				...
				if(currentType.getBits().isConstant(1))
					...
					sb.append(dataType+"[] "+v2+" = env.inputOfAlice(Utils.fromLong("+exp.x2.name+", "+this.currentType.getBits()+"));\n");
			}
...
//				ret = "new NullableType<"+dataType+", "+targetName(dt.type)+">(env, "+ con + ", "+bits+"[0])";
		...
		if(exp.getLabels() != Label.Pub) {
			...
			if(exp.x1.lab == Label.Pub) {
				...
				if(bits.isConstant(1))
					...
					sb.append(type+" "+v1+" = env.inputOfAlice(Utils.fromLong("+exp.x1.name+", "+bits+"));\n");
			}
			String v2 = exp.x2.name;
			if(exp.x2.lab == Label.Pub) {
				sb.append(indent(indent));
				...
				if(bits.isConstant(1))
					...
					sb.append(type+" "+v2+" = env.inputOfAlice(Utils.fromLong("+exp.x2.name+", "+bits+"));\n");
			}
			sb.append(indent(indent));
	...
	public Pair<String, String> visit(SopExp exp) {
		...
		if(exp.getLabels() != Label.Pub) {
			...
			sb.append(indent(indent));
			if(exp.op == SopExp.Op.Shl)
				sb.append("glong "+ret+" = "+exp.x1.name+" << "+exp.bit+";\n");
			else if(exp.op == SopExp.Op.Shr)
				sb.append("glong "+ret+" = "+exp.x1.name+" >> "+exp.bit+";\n");
	...
	public String visit(ArrayAssign arrayAssign) {
		if(arrayAssign.value.type instanceof NullType) {
			if(arrayAssign.indexLab == Label.Pub) {
				...
				if(arrayAssign.idx.lab == Label.Pub) {
					v1 = "env.inputOfAlice(Utils.fromLong("+arrayAssign.idx.name+", "+this.currentType.getBits()+"))";
				}
		...
		if(type.indexLab.lab != Label.Pub) {
			...
			if(arrayAssign.idx.lab == Label.Pub) {
				sb.append(indent(indent));
				v1 = "f_tmp_"+(this.tmpId++);
				sb.append(dataType+"[] "+v1+" = env.inputOfAlice(Utils.fromLong("+arrayAssign.idx.name+", "+this.currentType.getBits()+"));\n");
			}
			...
			if(arrayAssign.value.lab == Label.Pub) {
				if(arrayAssign.value.type instanceof IntType) {
					...
					sb.append(indent(indent));
					v2 = "f_tmp_"+(this.tmpId++);
					sb.append(dataType+"[] "+v2+" = env.inputOfAlice(Utils.fromLong("+arrayAssign.value.name+", "+this.currentType.getBits()+"));\n");
			...
			if(type.getLabel() != Label.Pub && arrayAssign.value.lab == Label.Pub) {
				...
				if(type.type instanceof IntType) {
					if(type.type.getBits().isConstant(1))
						...
						sb.append(visit(type.type)+" "+v+" = env.inputOfAlice(Utils.fromLong("+arrayAssign.value.name+", "+type.type.getBits()+"));\n");
				...
				if(assign.lab != Label.Pub && assign.exp.getLabels() == Label.Pub) {
					...
					if(assign.name.type instanceof IntType) {
						if(assign.name.type.getBits().isConstant(1))
							...
							sb.append(type + " "+assign.name.name+" = env.inputOfAlice(Utils.fromLong("+tmp.left+", "+assign.name.type.getBits()+"))");
					...
					if(visit(assign.name.type) == "long") {
						String temp_value = value;
						try {
							value = Long.toString(Long.parseLong(value)) + "L";
						}
						catch (Exception e) {
							value = temp_value;
						}
					}
					/*if(visit(assign.name.type) == "boolean") {
						String temp_value = value;
						 try {
...
//			sb.append(ty+" "+ret+" = ");
		...
		for(int i=0; i<exp.inputs.size(); ++i) {
			...
			if(exp.isNative) {
				if(var.type instanceof FunctionType) {
					...
					// JMD modification to accommodate 2-D arrays for sorting
					VariableConstant bits = at.type.getBits();
					String bitCount = bits.toString();
					if(bits instanceof Unknown) {
						VariableType vt = (VariableType) at.type;
						bitCount = "factory"+vt.name+".numBits()";
					}
		
					precomputing.append(indent(indent)+dataType+"[][] "+tmp+" = env.newTArray(" + at.size + "," + bitCount + ");\n");					
					precomputing.append(indent(indent)+"for(int _tmp_i=0; _tmp_i<"+at.size+"; ++_tmp_i) {\n");
					precomputing.append(indent(indent+1)+tmp+"[_tmp_i] = "+var.name+".read(intLib.toSignals(_tmp_i));\n");
			...
			} else { // TODO JMD: create a local function (as opposed to Native) for writing to honest broker
				if(var.type instanceof FunctionType && ((FunctionType)var.type).global) {
					...
					if(var.type.getLabel() == Label.Pub &&
						...
						if(var.type.getBits().isConstant(1))
							...
							value = "env.inputOfAlice(Utils.fromLong(" + var.name+", "+var.type.getBits()+"))";
					} else
						value = var.name;
		...
		if(value.contains("sortLib.sort")) {
		    StringBuffer postcomputing = new StringBuffer();
		    for(int i=0; i<exp.inputs.size(); ++i) {
			...
			if(var.type instanceof ArrayType) {
			    ArrayType at = (ArrayType)var.type;
			    postcomputing.append(";\n");
			    postcomputing.append(indent(indent)+"for(int _tmp_i=0; _tmp_i<"+at.size+"; ++_tmp_i) {\n");
			    postcomputing.append(indent(indent+1)+var.name+".write(intLib.toSignals(_tmp_i), " + tempArrays.get(tmpIdx) + "[_tmp_i]);\n");
			    ++tmpIdx;
			    postcomputing.append(indent(indent)+"}\n"); 
			    //Bugs.LOG.log("Postcomputing: " + postcomputing.toString());
			}
		    }
		    value = value + postcomputing.toString();
		}
		//Bugs.LOG.log(sb.toString());
		//Bugs.LOG.log(precomputing.toString());
	...
	public String visit(RecordAssign assign) {
		...
		if(type.fields.get(assign.field).getLabel() != Label.Pub && assign.value.lab == Label.Pub && assign.value.type instanceof IntType) {
			...
			if(type.fields.get(assign.field).getBits().isConstant(1))
				...
				sb.append(dataType+"[] "+v+" = env.inputOfAlice(Utils.fromLong("+assign.value.name+", "+type.fields.get(assign.field).getBits()+"));\n");
		}
			...
			if(var.getValue().lab == Label.Pub && exp.type.fields.get(var.getKey()).getLabel() == Label.Secure) {
				if(var.getValue().type instanceof IntType) {
					if(var.getValue().getBits().isConstant(1))
						...
						sb.append(ret+"."+var.getKey()+" = env.inputOfAlice(Utils.fromLong("+var.getValue()+", "+var.getValue().type.getBits()+"));\n");
				} else {
	...
	public String boxedConstructor(Type type) {
		...
		if(type instanceof IntType)
			con = "new BoxedLong<@dataType>(env, "+con+")";
		...
		if(type.typeParameter != null) {
			for(Type tt : type.typeParameter) {
				if(tt instanceof VariableType) {
					...
					if(withDef) sb.append("BoxedLong<"+dataType+"> ");
					sb.append(", ");
...
//		}
	...
	public Pair<String, String> visit(RangeExp exp) {
		if(exp.lab == Label.Pub) {
			...
			if(exp.rr == null)
				...
				return new Pair<String, String>("Arrays.copyOfRange("+exp.base+", (int) "+exp.rl+", (int) "+exp.rr+")", "");
		}
	}
	...
	public String visit(RangeAssign rangeAssign) {
		if(rangeAssign.lab == Label.Pub) {
			...
			if(rangeAssign.right == null) {
				...
				return indent(indent)+"System.arraycopy("+rangeAssign.value+", 0, "+rangeAssign.name+", (int) "+rangeAssign.left
						+", (int) (("+rangeAssign.right+")-("+rangeAssign.left+")));\n";
			}
		}
	}
		...
		if(exp.var.type instanceof NullType) {
			...
			if(exp.var.lab == Label.Pub) {
				if(exp.var.type instanceof FloatType)
					...
					value = "new BoxedLong<"+dataType+">(env, env.inputOfAlice(Utils.fromLong("+value+", " + exp.var.getBits() +")))";
				else if(exp.var.type instanceof RndType)
					value = "new BoxedRnd<"+dataType+">(env, env.inputOfAlice(Utils.fromLong("+value+", " + exp.var.getBits() +")))";
			} else {
				if(exp.var.type instanceof FloatType)
					value = "new BoxedFloat<"+dataType+">(env, "+value+")";
				else if(exp.var.type instanceof IntType)
					value = "new BoxedLong<"+dataType+">(env, "+value+")";
				else if(exp.var.type instanceof RndType)
	...
	public String visit(Debug ret) {
		StringBuffer sb = new StringBuffer();
		sb.append(indent(indent) + "if (env.mode != Mode.OPT) {\n");
		indent ++;
		...
		if(ret.cond != null) {
			...
			++indent;
			sb.append(indent(indent) + "condA =  env.outputToAlice(" + ret.cond.name +");\n");
			sb.append(indent(indent) + "condB =  env.outputToBob(" + ret.cond.name +");\n");
			...
			++indent;
			sb.append(indent(indent) + "condA = (Boolean)" + ret.cond.name + ";\n");
			sb.append(indent(indent) + "condB = (Boolean)" + ret.cond.name + ";\n");
		...
		if(ret.toShow.type instanceof IntType) {
			...
			if(!it.bit.isConstant(1)) {
				now = now.replaceAll("%x", "Utils.toLong(%x)");
			}
...
//			now = now.replaceAll("%x", "((Boolean[])%x)");
	...
	public String visit(SetNonNullArrayEntries ret) {
		StringBuffer sb = new StringBuffer();
		if(ret.code != null) {
			sb.append(visit(ret.code));
		}
		sb.append(indent(indent) + ret.name.name + ".setNonNullEntries("  + ret.value.name + ");\n");

		return sb.toString();
	}

	public String visit(GetNonNullArrayEntries ret) {
		StringBuffer sb = new StringBuffer();
		if(ret.code != null) {
			sb.append(visit(ret.code));
		}
		sb.append(indent(indent) + ret.value.name + " = " + ret.name.name + ".getNonNullEntries();\n");
		return sb.toString();
	}
	
	public String visit(WriteArray write) {
		
		return indent(indent)+"SecureArray.serialize(" +  write.array + ", " + write.filename + ");\n";
	
	}
	
	public String visit(ReadArray read) {

...
import com.oblivm.compiler.ast.type.ASTStringType;
...
//				visit(stmt);
	...
	public List<ASTType> visit(ASTArrayExpression arrayExpression) {
	    ASTType ty = assertOne(visit(arrayExpression.var)); // ty == idx
				...
				for(int i=0; i<type.inputTypes.size(); ++i) {
					...
					if(!t.canFlowTo(targetType)) {
						Bugs.LOG.log(exp.inputs.get(i).right.beginPosition,
								"cannot coerce the provided input to the target type " + t + " cannot map to " + targetType);
						return null;
					}
			...
			for(ASTFunction func : program.functionDef) {
				if(func.name.equals(name) && func.baseType == null) {
					...
					if(func.bitParameter.size() != funcExpression.bitParameters.size()) {
						Bugs.LOG.log(funcExpression.beginPosition, 
							     "2. numbers of bit parameters mis-match" + func.bitParameter.size() + " != " + funcExpression.bitParameters.size());
						return null;
					}
...
//			visit(binaryPredicate.right);
	...
	@Override
	public List<ASTType> visit(ASTStringConstantExpression constantExpression) {
		constantExpression.type = ASTStringType.get(constantExpression.bitSize, ASTLabel.Pub);
		return buildOne(constantExpression.type);
	}
	...
	public Boolean visit(ASTSetNonNullArrayEntriesStatement stmt) {
		ASTType type = assertOne(visit(stmt.expr));
		return type != null;
	}

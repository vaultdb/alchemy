...
public class TypeResolver extends DefaultVisitor<ASTStatement, ASTExpression, ASTType> implements ASTTypeVisitor<ASTType>, IFunctionVisitor<ASTFunction> {
	...
	public ASTStatement visit(ASTSetNonNullArrayEntriesStatement stmt) {
		if(copy) {
			stmt = new ASTSetNonNullArrayEntriesStatement(visit(stmt.var), visit(stmt.expr));
		} else {
			stmt.var = visit(stmt.var);			
			stmt.expr = visit(stmt.expr);
		}
		return stmt;
	}

	@Override
	public ASTStatement visit(ASTGetNonNullArrayEntriesStatement stmt) {
		if(copy) {
			stmt = new ASTGetNonNullArrayEntriesStatement(visit(stmt.var), visit(stmt.expr));
		} else {
			stmt.var = visit(stmt.var);			
			stmt.expr = visit(stmt.expr);
		}
	...
	@Override
	public ASTStatement visit(ASTWriteArrayStatement stmt) {
		if(copy) {
			stmt = new ASTWriteArrayStatement(stmt.array, stmt.dstFile);
		}
	...
	public ASTStatement visit(ASTReadArrayStatement stmt) {
		if(copy) {
			stmt = new ASTReadArrayStatement(stmt.array, stmt.srcFile);
		}

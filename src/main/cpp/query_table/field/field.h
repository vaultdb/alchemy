...
class Field {
    ...
    Field(const FieldType & field_type, const T & val)  : type_(field_type) {
            ...
            switch(type_) {
                ...
                case FieldType::SECURE_BOOL:
                    setBool(*((Bit *) &val));
                break;
                // TODO: add enforcer to only admit bit packing when SystemConfiguration indicates it is enabled
    ...
    explicit Field(const FieldType & field_type) : type_(field_type) {
        switch(type_) {
            case FieldType::BOOL:
                setValue<bool>(false);
                break;
    ...
    inline void setBool(const Bit & src) {
        payload_.resize(sizeof(Bit));
        memcpy(payload_.data(), &src, sizeof(Bit));
    }
    ...
    static void serialize(int8_t *dst, const Field<B> & f, const QueryFieldDesc & desc) {
        ...
        switch (f.getType()) {
            ...
            case FieldType::SECURE_BOOL: {
                memcpy(dst, f.payload_.data(), sizeof(emp::Bit));
                break;
            }
            case FieldType::SECURE_INT:
            case FieldType::SECURE_LONG:

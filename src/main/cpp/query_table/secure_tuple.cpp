...
void QueryTuple<emp::Bit>::compareSwap(const Bit &cmp, SecureTuple  & lhs, SecureTuple  & rhs) {
    ...
    // both sides have same size in bytes
    memcpy(lhs_payload.bits.data(), lhs.getData(), lhs.tuple_size_bytes_);
    memcpy(rhs_payload.bits.data(), rhs.getData(), lhs.tuple_size_bytes_);

    emp::swap(cmp, lhs_payload, rhs_payload);

    memcpy(lhs.getData(), lhs_payload.bits.data(), lhs.tuple_size_bytes_);
    memcpy(rhs.getData(), rhs_payload.bits.data(), lhs.tuple_size_bytes_);
...
QueryTuple<emp::Bit>::QueryTuple(QuerySchema *schema) {
    ...
    for (int i = 0; i < schema->size(); ++i) {
        managed_data_[i] = emp::Bit(0, emp::PUBLIC);
    }
...
SecureTuple QueryTuple<emp::Bit>::If(const emp::Bit &cond, const SecureTuple &lhs, const SecureTuple &rhs) {
    ...
    memcpy(dst.getData(), dst_int.bits.data(), lhs.tuple_size_bytes_);
    return dst;
}

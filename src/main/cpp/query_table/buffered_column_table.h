...
namespace vaultdb {
    class BufferedColumnTable : public QueryTable<Bit> {
        public:
            ...
            std::map<int, int64_t> serialized_col_bytes_offsets_;
...
namespace vaultdb {
    ...
    public:
        ...
        std::string file_path_ = this->conf_.stored_db_path_ + "/table_" + std::to_string(this->table_id_);
        std::string secret_shares_path_ = this->file_path_ + "/table_" + std::to_string(this->table_id_) + "." + std::to_string(this->conf_.party_);
        std::map<int, int64_t> serialized_col_bytes_offsets_; // bytes offsets for each column in the serialized file
        ...
        void setSchema(const QuerySchema &schema) override {
            ...
            this->serialized_col_bytes_offsets_[0] = 0;
            for (int i = 1; i < this->schema_.getFieldCount(); ++i) {
                col_bytes_cnt += (this->schema_.getField(i - 1).size() * this->tuple_cnt_) / 8;
                this->serialized_col_bytes_offsets_[i] = col_bytes_cnt;
            }
            col_bytes_cnt += (this->schema_.getField(this->schema_.getFieldCount() - 1).size() * this->tuple_cnt_) / 8;
            this->serialized_col_bytes_offsets_[-1] = col_bytes_cnt;
        }

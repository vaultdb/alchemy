...
QueryFieldDesc::QueryFieldDesc(const QueryFieldDesc &f, const int &  col_num)
        : field_name_(f.field_name_),
          ...
          field_size_bits_(f.field_size_bits_),
          field_min_(f.field_min_), bit_packed_size_(f.field_size_bits_), packed_wires_(f.packed_wires_)  {
}
...
QueryFieldDesc& QueryFieldDesc::operator=(const QueryFieldDesc& src)  {
    ...
    this->field_size_bits_ = src.field_size_bits_;
...
int QueryFieldDesc::sizeBytes() const {
        if (TypeUtilities::isSecretShared(type_)) {
            return field_size_bits_ * sizeof(Bit);
        }

        return field_size_bits_ / 8; // convert bits to bytes; padded to 8s earlier
}

void QueryFieldDesc::initializeFieldSize() {
    field_size_bits_ = TypeUtilities::getTypeSize(type_);
    bit_packed_size_ = field_size_bits_;
    ...
    if(table_name_ != "bit_packing") {
        ...
        if (type_ == FieldType::SECURE_INT || type_ == FieldType::SECURE_LONG
            ...
            if (type_ == FieldType::SECURE_INT || type_ == FieldType::SECURE_LONG) {
                field_size_bits_ = bit_packed_size_;
            }
        }
    }

    if(QueryFieldDesc::type_ == FieldType::STRING || QueryFieldDesc::type_ == FieldType::SECURE_STRING )  {
        field_size_bits_ *= string_length_;
        packed_wires_ = TypeUtilities::packedWireCount(field_size_bits_);
    }
}
...
void QueryFieldDesc::initializeFieldSizeWithCardinality(int cardinality) {
    ...
    this->field_size_bits_ = TypeUtilities::getTypeSize(type_);
    this->bit_packed_size_ =  this->field_size_bits_;
    this->field_min_ = 0;
    ...
    if(this->table_name_ != "bit_packing" && SystemConfiguration::getInstance().bitPackingEnabled()){
        ...
        if(bit_packed_size_ == 0) bit_packed_size_ = 1;
        this->field_size_bits_ = this->bit_packed_size_;
    }
...
bool QueryFieldDesc::bitPacked() const {
    return (field_size_bits_ != (int) TypeUtilities::getTypeSize(type_)) && (type_ == FieldType::SECURE_INT || type_ == FieldType::SECURE_LONG);
}

...
namespace  vaultdb {
    ...
    public:
        ...
        static RowTable<bool> *deserialize(const QuerySchema &schema, const vector<int8_t> &table_bytes);

        // encrypted version of deserialization using emp::Bit
        static RowTable<Bit> *deserialize(const QuerySchema &schema, vector<Bit> &table_bits);
...
//    std::ostream &operator<<(std::ostream &os, const RowTable<bool> &table);
//
//    std::ostream &operator<<(std::ostream &os, const RowTable<Bit> &table);

...
typedef    int32_t frame_id_t ;    // frame id type
...
enum class  StorageModel {
    ROW_STORE,
    COLUMN_STORE,
    BUFFER_FREE_COLUMN_STORE,
    OUTSOURCED_COLUMN_STORE,
    PACKED_COLUMN_STORE
};
...
typedef struct page_id_ {
    ...
    static PageId INVALID_PAGE_ID = PageId(-1024, -1024, -1024);
...
struct empBitSizes {
    const static int num_computing_party_ = emp::N;
...
namespace std {
    template <>
    struct hash<vaultdb::PageId> {
        size_t operator()(const vaultdb::PageId& obj) const {
            // Combine hashes of individual members
            size_t h1 = hash<int>{}(obj.table_id_);
            size_t h2 = hash<int>{}(obj.col_id_);
            size_t h3 = hash<int>{}(obj.page_idx_);

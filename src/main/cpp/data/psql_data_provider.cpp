...
PlainTable *PsqlDataProvider::getQueryTable(std::string db_name, std::string sql, const StorageModel &model, bool has_dummy_tag) {
    ...
    // plaintext always writes to ColumnTable
    QueryTable<bool> *dst_table = QueryTable<bool>::getTable(row_cnt, schema_, model);
...
void PsqlDataProvider::createTable(const string & db_name, const string & table_name,  const QuerySchema &schema) {
    string create_table = "CREATE TABLE IF NOT EXISTS " + table_name + "(";
    for(int i = 0; i < schema.getFieldCount(); ++i) {
        QueryFieldDesc field = schema.getField(i);
       create_table += field.getName() + " ";
        switch(field.getType()) {
            case FieldType::STRING:
                create_table += "char(" + std::to_string(field.getStringLength()) + ")";
                break;
            case FieldType::BOOL:
            ...
            case FieldType::LONG:
                ...
                break;
            default:
                throw std::invalid_argument("Unsupported field type" + TypeUtilities::getTypeName(field.getType()));
    ...
    create_table += ")";
    runQuery(db_name, create_table); // create the table in the database
}

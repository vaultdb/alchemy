    ...
    BOOST_FOREACH(boost::property_tree::ptree::value_type &v, pt.get_child("rels."))
                {
                    ...
                    int operator_id = v.second.get_child("id").get_value<int>();
...
Operator<B> *PlanParser<B>::parseSort(const int &operator_id, const boost::property_tree::ptree &sort_tree) {
    ...
    if(sort_tree.count("fetch") > 0) {
        string limit_str =  sort_tree.get_child("fetch.literal").get_value<string>();;
...
Operator<B> *PlanParser<B>::parseAggregate(const int &operator_id, const boost::property_tree::ptree &aggregate_json) {
    ...
    if(aggregate_json.count("checkSort") > 0) {
        check_sort = aggregate_json.get_child("checkSort").get_value<bool>();
    }
    else if(aggregate_json.count("check-sort") > 0) {
        check_sort = aggregate_json.get_child("check-sort").get_value<bool>();
    }
    ...
    if(aggregate_json.count("cardinality-bound") > 0)
        cardinality_bound_str = aggregate_json.get_child("cardinality-bound").get_value<string>();
    else if(aggregate_json.count("cardBound") > 0)
        cardinality_bound_str = aggregate_json.get_child("cardBound").get_value<string>();

    cardinality_bound = parseTableBound(cardinality_bound_str);

    if(aggregate_json.count("operator-algorithm") > 0)
        agg_algo = aggregate_json.get_child("operator-algorithm").get_value<string>();
    ...
    for (ptree::const_iterator it = agg_payload.begin(); it != agg_payload.end(); ++it) {
        ...
        s.alias = it->second.get_child("name").get_value<std::string>();
        s.is_distinct = !(it->second.get_child("distinct").get_value<std::string>() == "false");
    ...
    if(aggregate_json.count("input") > 0) {
        int lhs_id = aggregate_json.get_child("input").get_value<int>();
    ...
    if(!group_by_ordinals.empty()) {
        ...
        if (check_sort && sort_needed) {
            ...
            support_ops_.emplace_back(child);
        }
...
Operator<B> *PlanParser<B>::parseJoin(const int &operator_id, const ptree &join_tree) {
    ...
    if(join_tree.count("operator-algorithm") > 0)
        join_algo = join_tree.get_child("operator-algorithm").get_value<string>();
    ...
    if(join_tree.count("joinType") > 0)
        join_type = join_tree.get_child("joinType").get_value<string>();
    ...
    if(join_tree.count("foreignKey") > 0 || join_tree.count("foreign-key") > 0) {
        int foreign_key = (join_tree.count("foreignKey") > 0) ? join_tree.get_child("foreignKey").get_value<int>()
                                                              : join_tree.get_child("foreign-key").get_value<int>();
        ...
        if (join_algo == "keyed-sort-merge-join") {
            ...
            Operator<B>* lhs_sorter = nullptr;
            Operator<B>* rhs_sorter = nullptr;
            SortDefinition lhs_sort, rhs_sort;
            ...
            if (!lhs_sort_compatible && rhs_sort_compatible) {
                return new KeyedSortMergeJoin<B>(lhs_sorter, rhs, foreign_key, join_condition);
            } else if (lhs_sort_compatible && !rhs_sort_compatible) {
                return new KeyedSortMergeJoin<B>(lhs, rhs_sorter, foreign_key, join_condition);
            } else if (!lhs_sort_compatible && !rhs_sort_compatible) {
                return new KeyedSortMergeJoin<B>(lhs_sorter, rhs_sorter, foreign_key, join_condition);
            } else {
                return new KeyedSortMergeJoin<B>(lhs, rhs, foreign_key, join_condition);
            }
        }
    ...
    if (join_algo == "merge-join") {
        if(join_tree.count("dummy-handling") > 0 && join_tree.get_child("dummy-handling").get_value<string>() == "OR")
...
Operator<B> *PlanParser<B>::parseTableScan(const int & operator_id, const boost::property_tree::ptree &seq_scan_tree) {
    ...
    if(seq_scan_tree.count("table") > 0)
        table_name = seq_scan_tree.get_child("table").get_value<string>();
    assert(table_name != "");

    if(seq_scan_tree.count("input-limit") > 0)
        local_tuple_limit = seq_scan_tree.get_child("input-limit").get_value<int>();
...
Operator<B> *PlanParser<B>::parseStoredTableScan(const int & operator_id, const boost::property_tree::ptree &stored_table_scan_tree) {
    ...
    if(stored_table_scan_tree.count("table") == 1) {
        table_name = stored_table_scan_tree.get_child("table").get_value<string>();
    }
...
Operator<B> *PlanParser<B>::parseLocalScan(const int & operator_id, const boost::property_tree::ptree &local_scan_tree) {
    ...
    if(local_scan_tree.count("sql") > 0)
        sql = local_scan_tree.get_child("sql").get_value<string>();
    if(local_scan_tree.count("merge-sql") > 0)
        merge_sql = local_scan_tree.get_child("merge-sql").get_value<string>();
    if(local_scan_tree.count("operator-algorithm") > 0)
        op_algo = local_scan_tree.get_child("operator-algorithm").get_value<string>();

    if(local_scan_tree.count("party") > 0)
        input_party = local_scan_tree.get_child("party").get_value<int>();

    if(local_scan_tree.count("input-limit") > 0)
        local_tuple_limit = local_scan_tree.get_child("input-limit").get_value<int>();
    ...
    if(local_scan_tree.count("cardinality-bound") > 0)
        cardinality_bound = local_scan_tree.get_child("cardinality-bound").get_value<int>();
    ...
    if(op_algo == "merge-input" && std::is_same_v<B, Bit> && local_tuple_limit > 0 && SystemConfiguration::getInstance().emp_manager_->empMode() == EmpMode::SH2PC) {
...
Operator<B> *PlanParser<B>::parseShrinkwrap(const int & operator_id, const boost::property_tree::ptree &pt) {
    ...
    if(pt.count("output-cardinality") > 0)
        output_cardinality =  pt.get_child("output_cardinality").get_value<int>();

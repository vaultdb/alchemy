...
vector<int8_t> OutsourcedMpcManager::serializePackedWire(OMPCPackedWire &wire) const {
    int page_size_bits = system_conf_.bpm_.page_size_bytes_ / sizeof(emp::Bit);
    int block_n = (page_size_bits / 128) + (page_size_bits % 128 != 0);
    int packed_wire_size_bytes_ = (2 * block_n + 1) * sizeof(block); // 16 bytes per block = 128 bits
    ...
    memcpy(write_ptr, (int8_t *) wire.packed_masked_values.data(), block_n * sizeof(block));

    write_ptr += block_n * sizeof(block);
    memcpy(write_ptr, (int8_t *) wire.packed_lambdas.data(), block_n * sizeof(block));

    return dst;
}

emp::OMPCPackedWire OutsourcedMpcManager::deserializePackedWire(int8_t *serialized_packed_wire) const {
    int page_size_bits = system_conf_.bpm_.page_size_bytes_ / sizeof(emp::Bit);
    int block_n = (page_size_bits / 128) + (page_size_bits % 128 != 0);


    int8_t *read_ptr = serialized_packed_wire;
    int read_stride = block_n * sizeof(block);

    emp::OMPCPackedWire dst(block_n);
    ...
    if(party_ == system_conf_.input_party_) {
        ...
        for(int j = -1; j < dst_schema.getFieldCount(); ++j) {
            ...
            for (int i = 0; i < tuple_cnt; ++i) {
                ...
                this->flush(); // we are only doing this when we first initialize the DB, not during experiments, so it's ok if it is slow.

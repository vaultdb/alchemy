...
void encodeTable(string table_name) {
    ...
    PlainTable *table = DataUtilities::getQueryResults(db_name_, table_to_query_.at(table_name), StorageModel::BUFFER_FREE_COLUMN_STORE, false);
    ...
    conf_.bpm_.flushTable<Bit>(packed_table->table_id_);

    // if(!conf_.inputParty()) {
    packed_table->packed_pages_file_.flush();
    packed_table->packed_pages_file_.close();

    // write out shares
    string packed_pages_file = Utilities::getFilenameForTable(table_name);

    std::this_thread::sleep_for(std::chrono::seconds(1));

    std::filesystem::copy_file(packed_table->table_file_name_.c_str(), packed_pages_file.c_str(), std::filesystem::copy_options::overwrite_existing);

    std::this_thread::sleep_for(std::chrono::seconds(1));

    cout << "Wrote secret shares to " << packed_pages_file << endl;

    packed_table->packed_pages_file_.open(packed_table->table_file_name_.c_str(), std::ios::in | std::ios::out | std::ios::binary);
    // }
...
int main(int argc, char **argv) {
    ...
    cout << "Reading config file: " << conf_file << endl;


    conf_.setEmptyDbName(empty_db_);
    conf_.buffer_pool_enabled_ = true;
    conf_.bp_page_size_bytes_ = 2048 * sizeof(Bit);
    conf_.bp_page_cnt_ = 1e9 / (2048 * sizeof(Bit)); // about 1 GB of memory
...
//    BitPackingMetadata md = FieldUtilities::getBitPackingMetadata(argv[1]);
    ...
    if(VALIDATE) {
        cout << "Validating!\n";
        conf_.initializeWirePacking(dst_root_, temp_data_path_);

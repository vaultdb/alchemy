    ...
    static std::string getStackTrace();
        ...
        static string runCommand(const string & cmd, const string & cwd = "") {
            ...
            if (cmd.substr(0, 3) == "rm ") {
                string msg = "Prohibited behavior! attempting to delete with \" " + cmd + "\" in " + dir;
                // replace call with more conservative `filesystem::remove(filename)` 
        ...
        static string getStorageModeStr(const StorageModel & storage_model) {
            switch (storage_model) {
                ...
                case StorageModel::PACKED_COLUMN_STORE:
                    return "packed wires";
                case StorageModel::OUTSOURCED_COLUMN_STORE:
                    return "outsourced column store";
        ...
        static string getTestParameters() {
            ...
            output << "EMP mode: " << EmpManager::empModeString(s.emp_manager_->empMode()) << ", storage mode: " << getStorageModeStr(s.storageModel()) << endl;
        ...
        static inline block generateRandomBlock() {
            long lower, upper;
            int8_t *l_ptr = (int8_t *) &lower;
            int8_t *u_ptr = (int8_t *) &upper;
            ...
            return emp::makeBlock(upper, lower);
        }

        static inline int getPageCount( const int & field_size_bytes, const int & tuple_cnt, const int & page_size_bytes) {
            int fields_per_page = page_size_bytes / field_size_bytes;
            if (fields_per_page == 0) {
                throw std::runtime_error("Field size is larger than page size, cannot fit even one field in a page.");
            }
            return (tuple_cnt / fields_per_page) + (tuple_cnt % fields_per_page != 0);
        }

        static inline int pageAlignedBytes(const int & field_size_bytes, const int & tuple_cnt, const int & page_size_bytes) {
           return getPageCount(field_size_bytes, tuple_cnt, page_size_bytes) * page_size_bytes;
        }

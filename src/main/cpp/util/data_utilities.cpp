...
PlainTable *DataUtilities::getQueryResults(const std::string &db_name, const std::string &sql, const StorageModel &model,
                                           const bool &has_dummy_tag) {
    PsqlDataProvider dataProvider;
    return dataProvider.getQueryTable(db_name, sql, model, has_dummy_tag);
...
string DataUtilities::printColumn(const SecureTable *table, int col, int tuple_limit, bool show_dummies) {
    stringstream  ss;
    int cutoff = (tuple_limit > 0) ? tuple_limit : table->tuple_cnt_;
    ss << "(";
    if (cutoff < 1) {
        ss << ")";
        return ss.str();
    ...
    bool first = true;
    QueryFieldDesc desc = table->getSchema().getField(col);

    for (int i = 0; i < cutoff; ++i) {

        if (show_dummies || !(table->getDummyTag(i).reveal())) {
            if (!first) ss << ", ";
            ss << table->getField(i, col).reveal(desc).toString();
            first = false;
        }
    }
    ss << ")";
...
string DataUtilities::printColumn(const PlainTable *table, int col, int tuple_limit, bool show_dummies) {
    stringstream  ss;
    int cutoff = (tuple_limit > 0) ? tuple_limit : table->tuple_cnt_;
    ss << "(";
    if (cutoff < 1) {
    ...
    for (int i = 0; i < cutoff; ++i) {
        ...
        if (show_dummies || !(table->getDummyTag(i))) {
            if (!first) ss << ", ";
            ss << table->getField(i, col).toString();
            first = false;
        }
    }
    ss << ")";
...
template<typename B>
string DataUtilities::printFilteredTable(const QueryTable<B> *table, GenericExpression<B> *filter, int tuple_limit, bool show_dummies) {
    ...
    ss << table->getSchema() << '\n';
    for (int i = 0; i < table->tuple_cnt_; ++i) {
        if (printed >= tuple_limit && tuple_limit > 0) break;
        bool selected = FieldUtilities::extract_bool(filter->call(table, i).template getValue<B>());
        bool dummy = FieldUtilities::extract_bool(table->getDummyTag(i));
        if (selected && (show_dummies || !dummy)) {
            auto tuple = ((QueryTable<B> *)table)->revealRow(i); // const casting
            ss << i << ": " << tuple.toString(true) << endl;
            ++printed;
        }
    }
    return ss.str();
...
template string DataUtilities::printFilteredTable<bool>(const QueryTable<bool> *table, GenericExpression<bool> *filter, int tuple_limit, bool show_dummies);
template string DataUtilities::printFilteredTable<Bit>(const QueryTable<Bit> *table, GenericExpression<Bit> *filter, int tuple_limit, bool show_dummies);

...
//        static PlainTuple revealTuple(const SecureTuple  & s);
        ...
        static Field<bool> parseField(FieldType dst_type, const string & src, const int & str_len = 0) {
            switch (dst_type) {
                case FieldType::BOOL:
                    return Field<bool>(FieldType::BOOL, (src == "1" || src == "true"));
                case FieldType::INT:
                    return Field<bool>(FieldType::INT, std::stoi(src));
                case FieldType::LONG:
                    case FieldType::DATE:
                    return Field<bool>(FieldType::LONG, std::stoll(src));
                case FieldType::FLOAT:
                    return Field<bool>(FieldType::FLOAT, std::stof(src));
                case FieldType::STRING:
                    return Field<bool>(FieldType::STRING, src);
                default:
                    throw std::invalid_argument("Invalid field type for parsing: " + TypeUtilities::getTypeName(dst_type));
            }
        }

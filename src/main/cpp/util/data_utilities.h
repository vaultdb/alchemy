...
namespace vaultdb {
    ...
    template<typename B> class GenericExpression;
    ...
    public:
        ...
        static PlainTable *getQueryResults(const std::string &db_name, const std::string &sql, const StorageModel & model, const bool &has_dummy_tag = false);
...
//        static emp::Integer toEmpInteger(const vector<int8_t> & src_bytes);
    ...
    static string printColumn(const SecureTable *table, int col, int tuple_limit = -1, bool show_dummies = false);
    static string printColumn(const PlainTable *table, int col, int tuple_limit = -1, bool show_dummies = false);
    template<typename B>
    static string printFilteredTable(const QueryTable<B> *table, GenericExpression<B> *filter, int tuple_limit = -1, bool show_dummies = false);

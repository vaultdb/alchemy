...
TEST_F(PlanParserTest, tpch_q05) {
    ...
    "                    #5: KeyedJoin<bool> ($1 == $2) : (#0 int32 customer.c_nationkey, #1 int32 orders.o_orderkey, #2 int32 lineitem.l_orderkey, #3 int32 lineitem.l_suppkey, #4 float revenue) order by: {<2, ASC> , <0, ASC> }\n"
...
TEST_F(PlanParserTest, tpch_q08) {
    ...
    std::string expected_plan = "#11: Project<bool> ((<0, $0 int32>, <1, $2 / $1 float>)) : (#0 int32 orders.o_year, #1 float mkt_share) order by: {}\n"
    ...
    "                    #5: KeyedJoin<bool> ($0 == $2) : (#0 int32 orders.o_orderkey, #1 int32 orders.o_year, #2 int32 lineitem.l_orderkey, #3 int32 lineitem.l_suppkey, #4 float volume) order by: {<2, ASC> , <0, ASC> , <1, ASC> }\n"
...
TEST_F(PlanParserTest, tpch_q09) {
    ...
    "                                #2: KeyedJoin<bool> ($0 == $3) : (#0 int32 supplier.s_suppkey, #1 varchar(25) nation.n_name, #2 int32 partsupp.ps_partkey, #3 int32 partsupp.ps_suppkey, #4 float partsupp.ps_supplycost) order by: {<2, ASC> , <3, ASC> , <0, ASC> , <1, ASC> }\n"
...
TEST_F(PlanParserTest, tpch_q18) {
    ...
    "    #10: Project<bool> ((<0, $6 varchar>, <1, $5 int32>, <2, $0 int32>, <3, $3 int64>, <4, $2 float>, <5, $4 float>)) : (#0 varchar(25) customer.c_name, #1 int32 customer.c_custkey, #2 int32 order_keys.o_orderkey, #3 int64 orders.o_orderdate, #4 float orders.o_totalprice, #5 float sum_qty) order by: {<2, ASC> , <1, ASC> }\n"
    "        #9: KeyedJoin<bool> ($1 == $5) : (#0 int32 order_keys.o_orderkey, #1 int32 orders.o_custkey, #2 float orders.o_totalprice, #3 int64 orders.o_orderdate, #4 float sum_qty, #5 int32 customer.c_custkey, #6 varchar(25) customer.c_name) order by: {<0, ASC> , <5, ASC> }\n"

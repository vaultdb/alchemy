...
TEST_F(PlanParserTest, tpch_q08) {
    ...
    std::string expected_plan = "#11: Project<bool> ((<0, $0 int32>, <1, $2 / $1 float>)) : (#0 int32 orders.o_year, #1 float mkt_share) order by: {<0, ASC> }\n"
         ...
         "                            #4: SqlInput<bool> (\"SELECT  l_orderkey, l_suppkey, l_extendedprice * (1.0 - l_discount) volume, NOT p_type = 'LARGE ANODIZED STEEL' AS dummy_tag FROM part JOIN lineitem ON p_partkey = l_partkey ORDER BY l_orderkey, l_linenumber\", tuple_count=10) : (#0 int32 lineitem.l_orderkey, #1 int32 lineitem.l_suppkey, #2 float volume) order by: {<0, ASC> }\n"
...
TEST_F(PlanParserTest, tpch_q18) {
    ...
    std::string expected_plan = "#11: Sort<bool> ({<-1, ASC> , <4, DESC> , <3, ASC> }) : (#0 varchar(25) customer.c_name, #1 int32 customer.c_custkey, #2 int32 order_keys.o_orderkey, #3 int64 orders.o_orderdate, #4 float orders.o_totalprice, #5 float sum_qty) order by: {<-1, ASC> , <4, DESC> , <3, ASC> }\n"
         ...
         "                    #4: Filter<bool> ($1 > 7.000000) : (#0 int32 order_keys.o_orderkey, #1 float sum_qty) order by: {<0, ASC> }\n"

...
TEST_F(PlanParserTest, tpch_q5) {
    ...
    std::string expected_plan = "Sort<bool> ({<1, DESC> )) : (#0 varchar(25) nation.n_name, #1 float .revenue) order by: {<1, DESC> )\n"
                                ...
                                "                            SqlInput<bool> (\"SELECT * FROM (SELECT t1.n_name, t2.c_custkey, t2.c_nationkey FROM (SELECT r_regionkey, r_name FROM region WHERE r_name = 'EUROPE') AS t0 INNER JOIN (SELECT n_nationkey, n_name, n_regionkey FROM nation) AS t1 ON t0.r_regionkey = t1.n_regionkey INNER JOIN (SELECT c_custkey, c_nationkey FROM customer) AS t2 ON t1.n_nationkey = t2.c_nationkey WHERE c_custkey = 136 OR c_custkey = 31    OR c_custkey = 137 OR c_custkey = 32 ORDER BY t2.c_custkey ASC ) input LIMIT 10\", tuple_count=2) : (#0 varchar(25) nation.n_name, #1 int32 customer.c_custkey, #2 int32 customer.c_nationkey) order by: {<1, ASC> ) tuple count: 2\n"
                                "                            SqlInput<bool> (\"SELECT * FROM (SELECT o_orderkey, o_custkey, o_orderdate FROM orders WHERE o_orderkey = 4204389 OR o_orderkey = 3387013 OR o_orderkey = 4692995 ORDER BY o_orderkey, o_custkey, o_orderdate ) input LIMIT 10\", tuple_count=3) : (#0 int32 orders.o_orderkey, #1 int32 orders.o_custkey, #2 int64 .o_orderdate) order by: {<0, ASC> , <1, ASC> , <2, ASC> ) tuple count: 3\n"
                                "                        SqlInput<bool> (\"SELECT * FROM (SELECT l_orderkey, l_suppkey, l_extendedprice, l_discount FROM lineitem WHERE l_orderkey = 4204389 OR l_orderkey = 3387013 OR l_orderkey = 4692995    OR l_orderkey = 4204390 OR l_orderkey = 3387014 OR l_orderkey = 4692996 ORDER BY l_orderkey, l_suppkey, l_extendedprice, l_discount ) input LIMIT 10\", tuple_count=10) : (#0 int32 lineitem.l_orderkey, #1 int32 lineitem.l_suppkey, #2 float lineitem.l_extendedprice, #3 float lineitem.l_discount) order by: {<0, ASC> , <1, ASC> , <2, ASC> , <3, ASC> ) tuple count: 10\n"
                                "                    SqlInput<bool> (\"SELECT * FROM (SELECT t2.s_suppkey, t2.s_nationkey FROM (SELECT r_regionkey, r_name FROM region WHERE r_name = 'EUROPE') AS t0 INNER JOIN (SELECT n_nationkey, n_regionkey FROM nation) AS t1 ON t0.r_regionkey = t1.n_regionkey INNER JOIN (SELECT s_suppkey, s_nationkey FROM supplier) AS t2 ON t1.n_nationkey = t2.s_nationkey WHERE s_suppkey = 53 OR s_suppkey = 182 OR s_suppkey = 147    OR s_suppkey = 54 OR s_suppkey = 183 OR s_suppkey = 148 ORDER BY s_suppkey ) input LIMIT 10\", tuple_count=4) : (#0 int32 supplier.s_suppkey, #1 int32 supplier.s_nationkey) order by: {<0, ASC> ) tuple count: 4\n";
...
TEST_F(PlanParserTest, tpch_q8) {
    ...
    std::string expected_plan = "Project<bool> ((<0, $0 int32>, <1, $1 / $2 float>)) : (#0 int32 orders.o_year, #1 float .anonymous) order by: {<0, ASC> )\n"
                                ...
                                "                            SqlInput<bool> (\"SELECT * FROM (SELECT o_orderkey, o_custkey, o_orderyear::INT AS o_year FROM orders ORDER BY o_custkey, o_orderkey ) input LIMIT 10\", tuple_count=10) : (#0 int32 orders.o_orderkey, #1 int32 orders.o_custkey, #2 int32 orders.o_year) order by: {<1, ASC> , <0, ASC> ) tuple count: 10\n"
...
TEST_F(PlanParserTest, tpch_q18) {
    ...
    std::string expected_plan = "Sort<bool> ({<-1, ASC> , <4, DESC> , <3, ASC> )) : (#0 varchar(25) customer.c_name, #1 int32 customer.c_custkey, #2 int32 orders.o_orderkey, #3 int64 .o_orderdate, #4 float orders.o_totalprice, #5 float .sum_qty) order by: {<-1, ASC> , <4, DESC> , <3, ASC> )\n"
                                ...
                                "                                    Filter<bool> ($1 > 313) : (#0 int32 orders.o_orderkey, #1 int32 .null) order by: {<0, ASC> , <1, ASC> )\n"
                                "                                        GroupByAggregate<bool> (group-by: (0) aggs: (SUM($1) null)) : (#0 int32 orders.o_orderkey, #1 int32 .null) order by: {<0, ASC> , <1, ASC> )\n"
                                "                                            SqlInput<bool> (\"SELECT * FROM (SELECT o_orderkey, o_custkey, o_totalprice, o_orderdate FROM orders ORDER BY o_orderkey, o_custkey ) input LIMIT 10\", tuple_count=10) : (#0 int32 orders.o_orderkey, #1 int32 orders.o_custkey, #2 float orders.o_totalprice, #3 int64 .o_orderdate) order by: {<0, ASC> , <1, ASC> ) tuple count: 10\n"

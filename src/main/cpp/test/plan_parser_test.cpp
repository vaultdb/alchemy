...
TEST_F(PlanParserTest, tpch_q08) {
    ...
    std::string expected_plan = "#11: Project<bool> ((<0, $0 int32>, <1, $2 / $1 float>)) : (#0 int32 orders.o_year, #1 float .mkt_share) order by: {<0, ASC> }\n"
                                ...
                                "                #8: KeyedJoin<bool> ($0 == $2) : (#0 int32 orders.o_orderkey, #1 int32 orders.o_year, #2 int32 lineitem.l_orderkey, #3 float .volume, #4 float .selected_volume) order by: {<0, ASC> }\n"
                                "                    #3: Project<bool> ((<0, $1 int32>, <1, $3 int32>)) : (#0 int32 orders.o_orderkey, #1 int32 orders.o_year) order by: {<0, ASC> }\n"
                                "                        #2: KeyedJoin<bool> ($0 == $2) : (#0 int32 customer.c_custkey, #1 int32 orders.o_orderkey, #2 int32 orders.o_custkey, #3 int32 orders.o_year) order by: {<1, ASC> }\n"

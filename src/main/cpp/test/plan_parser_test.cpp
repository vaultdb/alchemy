...
TEST_F(PlanParserTest, tpch_q03) {
          ...
          "                            #0: SqlInput<bool> (\"SELECT * FROM (SELECT o_orderkey, o_custkey, o_orderdate, o_shippriority, NOT o_orderdate < DATE '1995-03-25' AS dummy_tag FROM orders ORDER BY o_orderkey, o_orderdate, o_shippriority) input LIMIT 10\", tuple_count=10) : (#0 int32 orders.o_orderkey, #1 int32 orders.o_custkey, #2 int64 orders.o_orderdate, #3 int32 orders.o_shippriority) order by: {<0, ASC> }\n"
          "                            #1: SqlInput<bool> (\"SELECT * FROM (SELECT c_custkey, NOT c_mktsegment = 'HOUSEHOLD ' AS dummy_tag FROM customer ORDER BY c_custkey) input LIMIT 10\", tuple_count=10) : (#0 int32 customer.c_custkey) order by: {<0, ASC> }\n"
...
TEST_F(PlanParserTest, tpch_q05) {
    std::string expected_plan =  "#11: Sort<bool> ({<1, DESC> }) : (#0 varchar(25) nation.n_name, #1 float revenue) order by: {<1, DESC> }\n"
          ...
          "                        #5: KeyedJoin<bool> ($1 == $2) : (#0 int32 customer.c_nationkey, #1 int32 orders.o_orderkey, #2 int32 lineitem.l_orderkey, #3 int32 lineitem.l_suppkey, #4 float revenue) order by: {<2, ASC> }\n"
          "                            #3: Project<bool> ((<0, $1 int32>, <1, $2 int32>)) : (#0 int32 customer.c_nationkey, #1 int32 orders.o_orderkey) order by: {<1, ASC> }\n"
          "                                #2: KeyedJoin<bool> ($0 == $3) : (#0 int32 customer.c_custkey, #1 int32 customer.c_nationkey, #2 int32 orders.o_orderkey, #3 int32 orders.o_custkey) order by: {<2, ASC> }\n"
          "                                    #0: SqlInput<bool> (\"SELECT * FROM (SELECT t2.c_custkey, t2.c_nationkey FROM (SELECT r_regionkey, r_name FROM region WHERE r_name = 'EUROPE') AS t0 INNER JOIN (SELECT n_nationkey, n_name, n_regionkey FROM nation) AS t1 ON t0.r_regionkey = t1.n_regionkey INNER JOIN (SELECT c_custkey, c_nationkey FROM customer) AS t2 ON t1.n_nationkey = t2.c_nationkey ORDER BY t2.c_custkey ASC) input LIMIT 10\", tuple_count=10) : (#0 int32 customer.c_custkey, #1 int32 customer.c_nationkey) order by: {<0, ASC> }\n"
          "                                    #1: SqlInput<bool> (\"SELECT * FROM (SELECT o_orderkey, o_custkey, NOT (o_orderdate >= DATE '1993-01-01' AND o_orderdate < DATE '1994-01-01') AS dummy_tag FROM orders ORDER BY o_orderkey, o_custkey, o_orderdate) input LIMIT 10\", tuple_count=10) : (#0 int32 orders.o_orderkey, #1 int32 orders.o_custkey) order by: {<0, ASC> }\n"
          "                            #4: SqlInput<bool> (\"SELECT * FROM (SELECT l_orderkey, l_suppkey, l_extendedprice * (1 - l_discount) AS revenue FROM lineitem ORDER BY l_orderkey, l_suppkey, revenue) input LIMIT 10\", tuple_count=10) : (#0 int32 lineitem.l_orderkey, #1 int32 lineitem.l_suppkey, #2 float revenue) order by: {<0, ASC> }\n"
          "                    #7: SqlInput<bool> (\"SELECT * FROM (SELECT s_suppkey, s_nationkey, n_name FROM supplier JOIN nation ON s_nationkey = n_nationkey JOIN region ON n_regionkey = r_regionkey WHERE r_name = 'EUROPE' ORDER BY s_suppkey) input LIMIT 10\", tuple_count=1) : (#0 int32 supplier.s_suppkey, #1 int32 supplier.s_nationkey, #2 varchar(25) nation.n_name) order by: {<0, ASC> }\n";
...
TEST_F(PlanParserTest, tpch_q08) {
    ...
    std::string expected_plan = "#11: Project<bool> ((<0, $0 int32>, <1, $2 / $1 float>)) : (#0 int32 orders.o_year, #1 float mkt_share) order by: {<0, ASC> }\n"
          ...
          "                        #5: KeyedJoin<bool> ($0 == $2) : (#0 int32 orders.o_orderkey, #1 int32 orders.o_year, #2 int32 lineitem.l_orderkey, #3 int32 lineitem.l_suppkey, #4 float volume) order by: {<2, ASC> }\n"
          ...
          "                    #7: SqlInput<bool> (\"SELECT * FROM (SELECT s_suppkey, CASE WHEN n_name = 'KENYA' THEN true ELSE false END AS nation_check FROM supplier JOIN nation ON n_nationkey = s_nationkey ORDER BY s_suppkey) input LIMIT 10\", tuple_count=10) : (#0 int32 supplier.s_suppkey, #1 bool nation_check) order by: {<0, ASC> }\n";
...
TEST_F(PlanParserTest, tpch_q09) {
    ...
    std::string expected_plan = "#11: Sort<bool> ({<0, ASC> , <1, DESC> }) : (#0 varchar(25) nation.nation, #1 int32 amount, #2 float sum_profit) order by: {<0, ASC> , <1, DESC> }\n"
          ...
          "                            #3: Project<bool> ((<0, $1 varchar>, <1, $2 int32>, <2, $3 int32>, <3, $4 float>)) : (#0 varchar(25) nation.nation, #1 int32 partsupp.ps_partkey, #2 int32 partsupp.ps_suppkey, #3 float partsupp.ps_supplycost) order by: {<1, ASC> , <2, ASC> }\n"
          "                                #2: KeyedJoin<bool> ($0 == $3) : (#0 int32 supplier.s_suppkey, #1 varchar(25) nation.n_name, #2 int32 partsupp.ps_partkey, #3 int32 partsupp.ps_suppkey, #4 float partsupp.ps_supplycost) order by: {<2, ASC> , <3, ASC> }\n"
...
TEST_F(PlanParserTest, tpch_q18) {
    ...
    std::string expected_plan = "#11: Sort<bool> ({<-1, ASC> , <4, DESC> , <3, ASC> }) : (#0 varchar(25) customer.c_name, #1 int32 customer.c_custkey, #2 int32 orders.o_orderkey, #3 int64 orders.o_orderdate, #4 float orders.o_totalprice, #5 float sum_qty) order by: {<-1, ASC> , <4, DESC> , <3, ASC> }\n"
          "    #10: Project<bool> ((<0, $6 varchar>, <1, $5 int32>, <2, $0 int32>, <3, $3 int64>, <4, $2 float>, <5, $4 float>)) : (#0 varchar(25) customer.c_name, #1 int32 customer.c_custkey, #2 int32 orders.o_orderkey, #3 int64 orders.o_orderdate, #4 float orders.o_totalprice, #5 float sum_qty) order by: {}\n"
          "        #9: KeyedJoin<bool> ($1 == $5) : (#0 int32 orders.o_orderkey, #1 int32 orders.o_custkey, #2 float orders.o_totalprice, #3 int64 orders.o_orderdate, #4 float sum_qty, #5 int32 customer.c_custkey, #6 varchar(25) customer.c_name) order by: {}\n"
          "            #7: Project<bool> ((<0, $2 int32>, <1, $3 int32>, <2, $4 float>, <3, $5 int64>, <4, $1 float>)) : (#0 int32 orders.o_orderkey, #1 int32 orders.o_custkey, #2 float orders.o_totalprice, #3 int64 orders.o_orderdate, #4 float sum_qty) order by: {}\n"
          "                #6: MergeJoin<bool> (no-op) : (#0 int32 order_keys.l_orderkey, #1 float sum_qty, #2 int32 orders.o_orderkey, #3 int32 orders.o_custkey, #4 float orders.o_totalprice, #5 int64 orders.o_orderdate) order by: {<0, ASC> }\n"
          "                    #4: Filter<bool> ($1 > 7.000000) : (#0 int32 order_keys.l_orderkey, #1 float sum_qty) order by: {<0, ASC> }\n"
          "                        #3: Project<bool> ((<0, $0 int32>, <1, $1 + $3 float>)) : (#0 int32 order_keys.l_orderkey, #1 float sum_qty) order by: {<0, ASC> }\n"
          "                            #2: MergeJoin<bool> (no-op) : (#0 int32 order_keys.l_orderkey, #1 float sum_qty, #2 int32 order_keys.l_orderkey, #3 float sum_qty) order by: {<0, ASC> }\n"
          "                                #0: SqlInput<bool> (\"SELECT * FROM (WITH all_keys AS (SELECT o_orderkey AS ak FROM order_keys),      sum_qtys AS (SELECT l_orderkey, sum(l_quantity) as sum_qty, false AS dummy_tag                   FROM lineitem                   GROUP BY l_orderkey                   ORDER BY l_orderkey) SELECT ak AS l_orderkey, COALESCE(sum_qty, 0.0) sum_qty, COALESCE(dummy_tag, true) dummy_tag FROM all_keys LEFT JOIN sum_qtys ON l_orderkey = ak ORDER BY ak ) input LIMIT 10\", tuple_count=10) : (#0 int32 order_keys.l_orderkey, #1 float sum_qty) order by: {<0, ASC> }\n"
          "                                #1: SqlInput<bool> (\"SELECT * FROM (WITH all_keys AS (SELECT o_orderkey AS ak FROM order_keys),      sum_qtys AS (SELECT l_orderkey, sum(l_quantity) as sum_qty, false AS dummy_tag                   FROM lineitem                   GROUP BY l_orderkey                   ORDER BY l_orderkey) SELECT ak AS l_orderkey, COALESCE(sum_qty, 0.0) sum_qty, COALESCE(dummy_tag, true) dummy_tag FROM all_keys LEFT JOIN sum_qtys ON l_orderkey = ak ORDER BY ak ) input LIMIT 10\", tuple_count=10) : (#0 int32 order_keys.l_orderkey, #1 float sum_qty) order by: {<0, ASC> }\n"
          "                    #5: SqlInput<bool> (\"SELECT * FROM (SELECT o_orderkey, o_custkey, o_totalprice, o_orderdate FROM orders ORDER BY o_orderkey) input LIMIT 10\", tuple_count=10) : (#0 int32 orders.o_orderkey, #1 int32 orders.o_custkey, #2 float orders.o_totalprice, #3 int64 orders.o_orderdate) order by: {<0, ASC> }\n"
          "            #8: SqlInput<bool> (\"SELECT * FROM (SELECT c_custkey, c_name FROM customer ORDER BY c_custkey) input LIMIT 10\", tuple_count=10) : (#0 int32 customer.c_custkey, #1 varchar(25) customer.c_name) order by: {<0, ASC> }\n";

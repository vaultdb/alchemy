...
TEST_F(PlanParserTest, tpch_q03) {
    ...
    SortDefinition expected_sort{ColumnSort(-1, SortDirection::ASCENDING),
           ...
           string expected_plan = "#9: Project<bool> ((<0, $0 int32>, <1, $3 float>, <2, $1 int64>, <3, $2 int32>)) : (#0 int32 orders.o_orderkey, #1 float .revenue, #2 int64 orders.o_orderdate, #3 int32 orders.o_shippriority) order by: {<-1, ASC> , <1, DESC> , <2, ASC> }\n"
                                ...
                                "                #5: KeyedJoin<bool> ($0 == $3) : (#0 int32 orders.o_orderkey, #1 int64 orders.o_orderdate, #2 int32 orders.o_shippriority, #3 int32 lineitem.l_orderkey, #4 float .revenue) order by: {<0, ASC> , <1, ASC> , <2, ASC> }\n"
...
TEST_F(PlanParserTest, tpch_q08) {
    ...
    std::string expected_plan = "#11: Project<bool> ((<0, $0 int32>, <1, $2 / $1 float>)) : (#0 int32 orders.o_year, #1 float .anonymous) order by: {<0, ASC> }\n"
                                ...
                                "        #9: Project<bool> ((<0, $1 int32>, <1, $3 float>, <2, $4 float>)) : (#0 int32 orders.o_year, #1 float .volume, #2 float .anonymous) order by: {<0, ASC> }\n"
                                "            #8: KeyedJoin<bool> ($0 == $2) : (#0 int32 orders.o_orderkey, #1 int32 orders.o_year, #2 int32 lineitem.l_orderkey, #3 float .volume, #4 float .anonymous) order by: {<1, ASC> , <0, ASC> }\n"
                                "                #3: Project<bool> ((<0, $0 int32>, <1, $2 int32>)) : (#0 int32 orders.o_orderkey, #1 int32 orders.o_year) order by: {<1, ASC> , <0, ASC> }\n"
                                "                    #2: KeyedJoin<bool> ($1 == $3) : (#0 int32 orders.o_orderkey, #1 int32 orders.o_custkey, #2 int32 orders.o_year, #3 int32 customer.c_custkey) order by: {<2, ASC> , <0, ASC> }\n"
                                ...
                                "                        #0: SqlInput<bool> (\"SELECT * FROM (SELECT c_custkey FROM customer c JOIN nation n1 ON c_nationkey = n_nationkey                 JOIN region ON r_regionkey = n_regionkey WHERE r_name = 'AFRICA' ORDER BY c_custkey ) input LIMIT 10\", tuple_count=10) : (#0 int32 customer.c_custkey) order by: {<1, ASC> }\n"
                                "                #7: Project<bool> ((<0, $0 int32>, <1, $2 float>, <2, CASE($4, $2, 0.000000) float>)) : (#0 int32 lineitem.l_orderkey, #1 float .volume, #2 float .anonymous) order by: {<0, ASC> }\n"
...
TEST_F(PlanParserTest, tpch_q09) {
    ...
    std::string expected_plan = "#11: Sort<bool> ({<0, ASC> , <1, DESC> }) : (#0 varchar(25) nation.n_name, #1 int32 .o_year, #2 float .sum_profit) order by: {<0, ASC> , <1, DESC> }\n"
                                ...
                                "                #8: KeyedJoin<bool> ($2 == $7) : (#0 varchar(25) nation.n_name, #1 float partsupp.ps_supplycost, #2 int32 lineitem.l_orderkey, #3 float lineitem.l_extendedprice, #4 float lineitem.l_discount, #5 float .l_quantity, #6 int32 .o_year, #7 int32 orders.o_orderkey) order by: {<0, ASC> }\n"
                                ...
                                "                                #2: KeyedJoin<bool> ($0 == $3) : (#0 int32 supplier.s_suppkey, #1 varchar(25) nation.n_name, #2 int32 partsupp.ps_partkey, #3 int32 partsupp.ps_suppkey, #4 float partsupp.ps_supplycost) order by: {<1, ASC> , <0, ASC> }\n"

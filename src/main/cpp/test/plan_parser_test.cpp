...
TEST_F(PlanParserTest, tpch_q09) {
    ...
    std::string expected_plan = "#11: Sort<bool> ({<0, ASC> , <1, DESC> }) : (#0 varchar(25) nation.nation, #1 int32 .o_year, #2 float .sum_profit) order by: {<0, ASC> , <1, DESC> }\n    #10: GroupByAggregate<bool> (group-by: (0, 1) aggs: (SUM($2) sum_profit)) : (#0 varchar(25) nation.nation, #1 int32 .o_year, #2 float .sum_profit) order by: {<0, ASC> , <1, ASC> }\n"
                                ...
                                "            #9: Project<bool> ((<0, $0 varchar>, <1, $6 int32>, <2, $3 * 1.000000 - $4 - $1 * $5 float>)) : (#0 varchar(25) nation.nation, #1 int32 .o_year, #2 float .amount) order by: {<0, ASC> }\n"
                                "                #8: KeyedJoin<bool> ($2 == $7) : (#0 varchar(25) nation.nation, #1 float partsupp.ps_supplycost, #2 int32 lineitem.l_orderkey, #3 float lineitem.l_extendedprice, #4 float lineitem.l_discount, #5 float .l_quantity, #6 int32 .o_year, #7 int32 orders.o_orderkey) order by: {<0, ASC> }\n"
                                "                    #6: Project<bool> ((<0, $0 varchar>, <1, $3 float>, <2, $4 int32>, <3, $8 float>, <4, $9 float>, <5, $7 float>)) : (#0 varchar(25) nation.nation, #1 float partsupp.ps_supplycost, #2 int32 lineitem.l_orderkey, #3 float lineitem.l_extendedprice, #4 float lineitem.l_discount, #5 float .l_quantity) order by: {<0, ASC> }\n"
                                ...
                                "                            #3: Project<bool> ((<0, $1 varchar>, <1, $2 int32>, <2, $3 int32>, <3, $4 float>)) : (#0 varchar(25) nation.nation, #1 int32 partsupp.ps_partkey, #2 int32 partsupp.ps_suppkey, #3 float partsupp.ps_supplycost) order by: {<0, ASC> }\n"

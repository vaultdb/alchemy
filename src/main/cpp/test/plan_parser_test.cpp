...
TEST_F(PlanParserTest, tpch_q1) {
    ...
    std::string expected_plan = "Sort<bool> ({<0, ASC> , <1, ASC> )) : (#0 varchar(1) lineitem.l_returnflag, #1 varchar(1) lineitem.l_linestatus, #2 float .sum_qty, #3 float .sum_base_price, #4 float .sum_disc_price, #5 float .sum_charge, #6 float .avg_qty, #7 float .avg_price, #8 float .avg_disc, #9 int64 .count_order) order by: {<0, ASC> , <1, ASC> )\n"
                                ...
                                "        SqlInput<bool> (\"SELECT * FROM (SELECT l_returnflag, l_linestatus, l_quantity, l_extendedprice, l_extendedprice * (1 - l_discount) AS discount, l_extendedprice * (1 - l_discount) * (1 + l_tax) AS charge, l_discount, NOT l_shipdate <= DATE '1998-08-03' AS dummy_tag FROM lineitem ORDER BY l_orderkey, l_linenumber ) input LIMIT 10\", tuple_count=10) : (#0 varchar(1) lineitem.l_returnflag, #1 varchar(1) lineitem.l_linestatus, #2 float lineitem.l_quantity, #3 float lineitem.l_extendedprice, #4 float .discount, #5 float .charge, #6 float lineitem.l_discount) order by: {<0, ASC> , <1, ASC> ) tuple count: 10\n";
...
TEST_F(PlanParserTest, tpch_q3) {
    ...
    std::string expected_plan = "Project<bool> ((<0, $0 int32>, <1, $3 float>, <2, $1 int64>, <3, $2 int32>)) : (#0 int32 lineitem.l_orderkey, #1 float .revenue, #2 int64 .o_orderdate, #3 int32 orders.o_shippriority) order by: {<-1, ASC> , <1, DESC> , <2, ASC> )\n"
                                ...
                                "                    BasicJoin<bool> ($6 == $2) : (#0 int32 customer.c_custkey, #1 varchar(10) customer.c_mktsegment, #2 int32 orders.o_orderkey, #3 int32 orders.o_custkey, #4 int64 .o_orderdate, #5 int32 orders.o_shippriority, #6 int32 lineitem.l_orderkey, #7 int64 .l_shipdate, #8 float .revenue) order by: {)\n"
                                "                        BasicJoin<bool> ($0 == $3) : (#0 int32 customer.c_custkey, #1 varchar(10) customer.c_mktsegment, #2 int32 orders.o_orderkey, #3 int32 orders.o_custkey, #4 int64 .o_orderdate, #5 int32 orders.o_shippriority) order by: {)\n"
...
TEST_F(PlanParserTest, tpch_q5) {
    ...
    std::string expected_plan = "Sort<bool> ({<1, DESC> )) : (#0 varchar(25) nation.n_name, #1 float .revenue) order by: {<1, DESC> )\n"
                                ...
                                "                BasicJoin<bool> ($7 == $10 AND $2 == $11) : (#0 varchar(25) nation.n_name, #1 int32 customer.c_custkey, #2 int32 customer.c_nationkey, #3 int32 orders.o_orderkey, #4 int32 orders.o_custkey, #5 int64 .o_orderdate, #6 int32 lineitem.l_orderkey, #7 int32 lineitem.l_suppkey, #8 float lineitem.l_extendedprice, #9 float lineitem.l_discount, #10 int32 supplier.s_suppkey, #11 int32 supplier.s_nationkey) order by: {)\n"
                                "                    BasicJoin<bool> ($6 == $3) : (#0 varchar(25) nation.n_name, #1 int32 customer.c_custkey, #2 int32 customer.c_nationkey, #3 int32 orders.o_orderkey, #4 int32 orders.o_custkey, #5 int64 .o_orderdate, #6 int32 lineitem.l_orderkey, #7 int32 lineitem.l_suppkey, #8 float lineitem.l_extendedprice, #9 float lineitem.l_discount) order by: {)\n"
                                "                        BasicJoin<bool> ($1 == $4) : (#0 varchar(25) nation.n_name, #1 int32 customer.c_custkey, #2 int32 customer.c_nationkey, #3 int32 orders.o_orderkey, #4 int32 orders.o_custkey, #5 int64 .o_orderdate) order by: {)\n"
...
TEST_F(PlanParserTest, tpch_q8) {
    ...
    std::string expected_plan = "Project<bool> ((<0, $0 int32>, <1, $1 / $2 float>)) : (#0 int32 orders.o_year, #1 float .anonymous) order by: {<0, ASC> )\n"
                                ...
                                "                    BasicJoin<bool> ($2 == $5) : (#0 int32 customer.c_nationkey, #1 int32 customer.c_custkey, #2 int32 orders.o_orderkey, #3 int32 orders.o_custkey, #4 int32 orders.o_year, #5 int32 lineitem.l_partkey, #6 int32 lineitem.l_orderkey, #7 int32 lineitem.l_suppkey, #8 float .volume, #9 int32 supplier.s_suppkey, #10 float .nation_check) order by: {)\n"
                                "                        BasicJoin<bool> ($3 == $1) : (#0 int32 customer.c_nationkey, #1 int32 customer.c_custkey, #2 int32 orders.o_orderkey, #3 int32 orders.o_custkey, #4 int32 orders.o_year) order by: {)\n"
                                ...
                                "                            SqlInput<bool> (\"SELECT * FROM (SELECT o_orderkey, o_custkey, o_orderyear AS o_year, NOT (o_orderdate >= DATE '1995-01-01' AND o_orderdate <= DATE '1996-12-31') AS dummy_tag FROM orders ORDER BY o_orderkey ) input LIMIT 10\", tuple_count=10) : (#0 int32 orders.o_orderkey, #1 int32 orders.o_custkey, #2 int32 orders.o_year) order by: {<0, ASC> ) tuple count: 10\n"
                                "                        BasicJoin<bool> ($4 == $2) : (#0 int32 lineitem.l_partkey, #1 int32 lineitem.l_orderkey, #2 int32 lineitem.l_suppkey, #3 float .volume, #4 int32 supplier.s_suppkey, #5 float .nation_check) order by: {)\n"
...
TEST_F(PlanParserTest, tpch_q18) {
    ...
    std::string expected_plan = "Sort<bool> ({<-1, ASC> , <4, DESC> , <3, ASC> )) : (#0 varchar(25) customer.c_name, #1 int32 customer.c_custkey, #2 int32 orders.o_orderkey, #3 int64 .o_orderdate, #4 float orders.o_totalprice, #5 float .sum_qty) order by: {<-1, ASC> , <4, DESC> , <3, ASC> )\n"
                                ...
                                "                BasicJoin<bool> ($6 == $3) : (#0 int32 lineitem.l_orderkey, #1 float lineitem.l_quantity, #2 int32 orders.o_orderkey, #3 int32 orders.o_custkey, #4 int64 .o_orderdate, #5 float orders.o_totalprice, #6 int32 customer.c_custkey, #7 varchar(25) customer.c_name) order by: {)\n"
                                "                    BasicJoin<bool> ($0 == $2) : (#0 int32 lineitem.l_orderkey, #1 float lineitem.l_quantity, #2 int32 orders.o_orderkey, #3 int32 orders.o_custkey, #4 int64 .o_orderdate, #5 float orders.o_totalprice) order by: {)\n"
                                ...
                                "                            BasicJoin<bool> ($0 == $4) : (#0 int32 orders.o_orderkey, #1 int32 orders.o_custkey, #2 float orders.o_totalprice, #3 int64 .o_orderdate, #4 int32 orders.o_orderkey) order by: {)\n"

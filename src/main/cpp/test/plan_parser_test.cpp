...
TEST_F(PlanParserTest, tpch_q3) {
    ...
    std::string expected_plan = "#8: Project<bool> ((<0, $0 int32>, <1, $3 float>, <2, $1 int64>, <3, $2 int32>)) : (#0 int32 lineitem.l_orderkey, #1 float .revenue, #2 int64 .o_orderdate, #3 int32 orders.o_shippriority) order by: {<-1, ASC> , <1, DESC> , <2, ASC> )\n"
                                "    #7: Sort<bool> ({<-1, ASC> , <3, DESC> , <1, ASC> )) : (#0 int32 lineitem.l_orderkey, #1 int64 .o_orderdate, #2 int32 orders.o_shippriority, #3 float .revenue) order by: {<-1, ASC> , <3, DESC> , <1, ASC> )\n"
                                "        #6: GroupByAggregate<bool> (group-by: (0, 1, 2) aggs: (SUM($3) revenue)) : (#0 int32 lineitem.l_orderkey, #1 int64 .o_orderdate, #2 int32 orders.o_shippriority, #3 float .revenue) order by: {<0, ASC> , <1, ASC> , <2, ASC> )\n"
                                "            #-1: Sort<bool> ({<0, ASC> , <1, ASC> , <2, ASC> )) : (#0 int32 lineitem.l_orderkey, #1 int64 .o_orderdate, #2 int32 orders.o_shippriority, #3 float .revenue) order by: {<0, ASC> , <1, ASC> , <2, ASC> )\n"
                                "                #5: Project<bool> ((<0, $6 int32>, <1, $4 int64>, <2, $5 int32>, <3, $8 float>)) : (#0 int32 lineitem.l_orderkey, #1 int64 .o_orderdate, #2 int32 orders.o_shippriority, #3 float .revenue) order by: {)\n"
                                "                    #4: KeyedJoin<bool> ($6 == $2) : (#0 int32 customer.c_custkey, #1 varchar(10) customer.c_mktsegment, #2 int32 orders.o_orderkey, #3 int32 orders.o_custkey, #4 int64 .o_orderdate, #5 int32 orders.o_shippriority, #6 int32 lineitem.l_orderkey, #7 int64 .l_shipdate, #8 float .revenue) order by: {)\n"
                                "                        #2: KeyedJoin<bool> ($0 == $3) : (#0 int32 customer.c_custkey, #1 varchar(10) customer.c_mktsegment, #2 int32 orders.o_orderkey, #3 int32 orders.o_custkey, #4 int64 .o_orderdate, #5 int32 orders.o_shippriority) order by: {)\n"
                                ...
                                "                            #1: SqlInput<bool> (\"SELECT * FROM (SELECT o_orderkey, o_custkey, o_orderdate, o_shippriority, NOT o_orderdate < DATE '1995-03-25' AS dummy_tag FROM orders ORDER BY o_orderkey, o_orderdate, o_shippriority ) input LIMIT 10\", tuple_count=10) : (#0 int32 orders.o_orderkey, #1 int32 orders.o_custkey, #2 int64 .o_orderdate, #3 int32 orders.o_shippriority) order by: {<0, ASC> , <2, ASC> , <3, ASC> )\n"
                                "                        #3: SqlInput<bool> (\"SELECT * FROM (SELECT l_orderkey, l_shipdate, l_extendedprice * (1 - l_discount) AS revenue, NOT l_shipdate > DATE '1995-03-25' AS dummy_tag FROM lineitem ORDER BY l_orderkey, l_shipdate ) input LIMIT 10\", tuple_count=10) : (#0 int32 lineitem.l_orderkey, #1 int64 .l_shipdate, #2 float .revenue) order by: {<0, ASC> , <1, ASC> )\n";
...
TEST_F(PlanParserTest, tpch_q5) {
    ...
    std::string expected_plan = "#9: Sort<bool> ({<1, DESC> )) : (#0 varchar(25) nation.n_name, #1 float .revenue) order by: {<1, DESC> )\n"
                                ...
                                "                    #4: KeyedJoin<bool> ($6 == $3) : (#0 varchar(25) nation.n_name, #1 int32 customer.c_custkey, #2 int32 customer.c_nationkey, #3 int32 orders.o_orderkey, #4 int32 orders.o_custkey, #5 int64 .o_orderdate, #6 int32 lineitem.l_orderkey, #7 int32 lineitem.l_suppkey, #8 float lineitem.l_extendedprice, #9 float lineitem.l_discount) order by: {)\n"
                                "                        #2: KeyedJoin<bool> ($1 == $4) : (#0 varchar(25) nation.n_name, #1 int32 customer.c_custkey, #2 int32 customer.c_nationkey, #3 int32 orders.o_orderkey, #4 int32 orders.o_custkey, #5 int64 .o_orderdate) order by: {)\n"
...
TEST_F(PlanParserTest, tpch_q8) {
    ...
    std::string expected_plan = "#10: Project<bool> ((<0, $0 int32>, <1, $1 / $2 float>)) : (#0 int32 orders.o_year, #1 float .anonymous) order by: {<0, ASC> )\n"
                                ...
                                "                            #0: SqlInput<bool> (\"SELECT * FROM (SELECT c_nationkey, c_custkey FROM customer c JOIN nation n1 ON c_nationkey = n_nationkey                 JOIN region ON r_regionkey = n_regionkey WHERE r_name = 'MIDDLE EAST' ORDER BY c_custkey ) input LIMIT 10\", tuple_count=10) : (#0 int32 customer.c_nationkey, #1 int32 customer.c_custkey) order by: {<1, ASC> )\n"
                                "                            #1: SqlInput<bool> (\"SELECT * FROM (SELECT o_orderkey, o_custkey, o_orderyear::INT AS o_year FROM orders ORDER BY o_custkey, o_orderkey ) input LIMIT 10\", tuple_count=10) : (#0 int32 orders.o_orderkey, #1 int32 orders.o_custkey, #2 int32 orders.o_year) order by: {<1, ASC> , <0, ASC> )\n"
                                ...
                                "                            #3: SqlInput<bool> (\"SELECT * FROM (SELECT l_partkey, l_orderkey, l_suppkey, l_extendedprice * (1.0 - l_discount) volume FROM part JOIN lineitem ON p_partkey = l_partkey WHERE p_type = 'PROMO BRUSHED COPPER' ORDER BY l_orderkey ) input LIMIT 10\", tuple_count=10) : (#0 int32 lineitem.l_partkey, #1 int32 lineitem.l_orderkey, #2 int32 lineitem.l_suppkey, #3 float .volume) order by: {<1, ASC> )\n"
                                "                            #4: SqlInput<bool> (\"SELECT * FROM (SELECT s_suppkey, CASE WHEN n_name = 'EGYPT' THEN 1.0 ELSE 0.0 END AS nation_check FROM supplier JOIN nation ON n_nationkey = s_nationkey ORDER BY s_suppkey ) input LIMIT 10\", tuple_count=10) : (#0 int32 supplier.s_suppkey, #1 float .nation_check) order by: {<0, ASC> )\n";
...
TEST_F(PlanParserTest, tpch_q18) {
    ...
    std::string expected_plan = "#13: Sort<bool> ({<-1, ASC> , <4, DESC> , <3, ASC> )) : (#0 varchar(25) customer.c_name, #1 int32 customer.c_custkey, #2 int32 orders.o_orderkey, #3 int64 .o_orderdate, #4 float orders.o_totalprice, #5 float .sum_qty) order by: {<-1, ASC> , <4, DESC> , <3, ASC> )\n"
                                ...
                                "                        #7: SqlInput<bool> (\"SELECT * FROM (SELECT l_orderkey, l_quantity FROM lineitem ORDER BY l_orderkey, l_quantity ) input LIMIT 10\", tuple_count=10) : (#0 int32 lineitem.l_orderkey, #1 float lineitem.l_quantity) order by: {<0, ASC> , <1, ASC> )\n"
                                ...
                                "                                    #3: Filter<bool> ($1 > 300.000000) : (#0 int32 lineitem.l_orderkey, #1 float .null) order by: {<0, ASC> , <1, ASC> )\n"
                                "                                        #2: GroupByAggregate<bool> (group-by: (0) aggs: (SUM($1) null)) : (#0 int32 lineitem.l_orderkey, #1 float .null) order by: {<0, ASC> , <1, ASC> )\n"
                                "                                            #0: SqlInput<bool> (\"SELECT * FROM (SELECT l_orderkey, l_quantity FROM lineitem ORDER BY l_orderkey, l_quantity ) input LIMIT 10\", tuple_count=10) : (#0 int32 lineitem.l_orderkey, #1 float lineitem.l_quantity) order by: {<0, ASC> , <1, ASC> )\n"

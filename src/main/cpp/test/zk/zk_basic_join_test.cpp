...
TEST_F(ZkBasicJoinTest, test_tpch_q3_customer_orders) {
    ...
    BasicJoin join(orders_input, customer_input, customer_orders_predicate);
...
TEST_F(ZkBasicJoinTest, test_tpch_q3_lineitem_orders) {
...
std::string expectedResultSql = "WITH orders_cte AS (" + orders_sql + "), \n"
    ...
    BasicJoin join(lineitem_input, orders_input, lineitem_orders_predicate);

...
public:
  ...
  static std::string getInputQuery() {
    ...
    static const std::string inputQuery =
        "SELECT l_orderkey, l_comment, l_returnflag, l_discount, "
        "EXTRACT(EPOCH FROM l_commitdate) AS l_commitdate " // handle timestamps
                                                            // by converting
                                                            // them to longs
                                                            // using SQL -
                                                            // "CAST(EXTRACT(EPOCH
  ...
  static const std::unique_ptr<QueryTable> getExpectedOutput() {
    ...
    return DataUtilities::getUnionedResults(getInputQuery(), "tpch_alice",
                                            "tpch_bob", false);
  }
  ...
  static std::string getInputQueryDummyTag() {
    ...
    static const std::string inputQueryDummyTag =
        "SELECT l_orderkey, l_comment, l_returnflag, l_discount, "
        "EXTRACT(EPOCH FROM l_commitdate) AS l_commitdate, " // handle
                                                             // timestamps by
                                                             // converting them
                                                             // to longs using
                                                             // SQL -
                                                             ...
                                                             // BIGINT) AS
                                                             // l_commitdate,
        "l_returnflag <> 'N' AS dummy " // simulate a filter for l_returnflag =
                                        // 'N' -- all of the ones that dont
                                        // match are dummies
        "FROM lineitem "
        "ORDER BY l_orderkey "
        "LIMIT 5";
  ...
  static const std::unique_ptr<QueryTable> getExpectedOutputDummyTag() {
    ...
    return DataUtilities::getUnionedResults(getInputQueryDummyTag(),
                                            "tpch_alice", "tpch_bob", false);
  }
  virtual void SetUp() {}
};
...
TEST_F(EmpManagerTest, emp_manager_test_int) {
  ...
  empManager->configureEmpManager(FLAGS_alice_host.c_str(), FLAGS_port,
                                  FLAGS_party);
...
TEST_F(EmpManagerTest, emp_manager_test_varchar) {
  ...
  empManager->configureEmpManager(FLAGS_alice_host.c_str(), FLAGS_port,
                                  FLAGS_party);
  // std::cout << "Received emp manager at " << empManager->toString() <<
  // std::endl;
  ...
  bool *bools =
      DataUtilities::bytesToBool((int8_t *)initialString.c_str(), stringLength);
  // encrypting as ALICE
  emp::Integer aliceSecretShared(stringBitCount, 0L, emp::ALICE);
  if (FLAGS_party == emp::ALICE) {
    ProtocolExecution::prot_exec->feed((block *)aliceSecretShared.bits.data(),
                                       emp::ALICE, bools, stringBitCount);
  } else {
    ProtocolExecution::prot_exec->feed((block *)aliceSecretShared.bits.data(),
                                       emp::ALICE, nullptr, stringBitCount);
  }
  empManager->flush();
  delete[] bools;
  // the standard reveal method converts this to decimal.  Need to reveal it
  // bitwise
  bools = new bool[stringBitCount];
  ProtocolExecution::prot_exec->reveal(bools, emp::PUBLIC,
                                       (block *)aliceSecretShared.bits.data(),
                                       stringBitCount);
  char *decodedBytes =
      (char *)DataUtilities::boolsToBytes(bools, stringBitCount);
...
TEST_F(EmpManagerTest, encrypt_table_one_column) {
  ...
  std::string inputQuery = "SELECT l_orderkey FROM lineitem ORDER BY "
                           "l_orderkey, l_linenumber LIMIT 2";
  std::cout << "Querying " << db_name << " at " << FLAGS_alice_host << ":"
            << FLAGS_port << " with: " << inputQuery << std::endl;

  std::unique_ptr<QueryTable> inputTable =
      dataProvider.getQueryTable(db_name, inputQuery, false);

  std::cout << "Initial table: " << *inputTable << std::endl;
  std::shared_ptr<QueryTable> encryptedTable =
      empManager->secretShareTable(inputTable.get());

  std::cout << "Finished encrypting table with "
            << encryptedTable->getTupleCount() << " tuples." << std::endl;

  empManager->flush();

  std::unique_ptr<QueryTable> expectedTable = DataUtilities::getUnionedResults(
      "tpch_alice", "tpch_bob", inputQuery, false);

  std::unique_ptr<QueryTable> decryptedTable =
      encryptedTable->reveal(emp::PUBLIC);

  std::cout << "Observed: \n" << *decryptedTable << endl;

  ASSERT_EQ(*expectedTable, *decryptedTable)
      << "Query table was not processed correctly.";
...
TEST_F(EmpManagerTest, encrypt_table_two_cols) {
  ...
  ASSERT_EQ(expectedTable->toString(),
            decryptedTable->toString()); // check the obvious stuff first
  ASSERT_EQ(*expectedTable, *decryptedTable)
      << "Query table was not processed correctly.";
...
TEST_F(EmpManagerTest, encrypt_table) {
  ...
  std::unique_ptr<QueryTable> expectedTable = DataUtilities::getUnionedResults(
      "tpch_alice", "tpch_bob", EmpManagerTestEnvironment::getInputQuery(),
      false);
...
TEST_F(EmpManagerTest, encrypt_table_dummy_tag) {
  ...
  std::unique_ptr<QueryTable> inputTable =
      dataProvider.getQueryTable(db_name, inputQuery, true);
  ...
  std::unique_ptr<QueryTable> expectedTable = DataUtilities::getUnionedResults(
      "tpch_alice", "tpch_bob", inputQuery, true);

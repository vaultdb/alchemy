...
namespace vaultdb {
    ...
    std::map<int, std::string> tpch_queries = {
            ...
            {5, "SELECT n.n_name, SUM(l.l_extendedprice * (1 - l.l_discount)) as revenue\n"
                ...
                "WHERE c.c_nationkey = s.s_nationkey  AND r.r_name = 'MIDDLE EAST ' AND o.o_orderdate >= date '1993-01-01' AND o.o_orderdate < date '1994-01-01'\n"
                " GROUP BY   n.n_name\n"
    ...
    std::map<int, std::string> truncated_tpch_queries_single_input_party = {
            {1, "SELECT  l_returnflag,   l_linestatus, SUM(l_quantity) as sum_qty, SUM(l_extendedprice) as sum_base_price,   SUM(l_extendedprice * (1 - l_discount)) as sum_disc_price,  SUM(l_extendedprice * (1 - l_discount) * (1 + l_tax)) as sum_charge,  AVG(l_quantity) as avg_qty, AVG(l_extendedprice) as avg_price,   AVG(l_discount) as avg_disc,  COUNT(*) as count_order \n"
                "FROM  (SELECT * FROM  lineitem ORDER BY  l_returnflag, l_linestatus, l_orderkey, l_linenumber LIMIT $LIMIT) t1 \n"
                " WHERE  l_shipdate <= date '1998-08-03' \n"
                "GROUP BY  l_returnflag,  l_linestatus \n"
                "ORDER BY   l_returnflag,  l_linestatus"},
            ...
            {8, "WITH  supplier_nation AS (\n"
                ...
                "		FROM nation JOIN supplier  ON n_nationkey = s_nationkey\n"
                "       ORDER BY s_suppkey LIMIT $LIMIT),\n"
                " customer_region AS (SELECT c_nationkey, c_custkey FROM  customer  JOIN nation ON c_nationkey = n_nationkey \n"
                "                            JOIN region on r_regionkey = n_regionkey WHERE  r_name='AFRICA' ORDER BY  c_custkey  LIMIT $LIMIT), \n"
                "  lineitem_part AS (SELECT l_partkey, l_orderkey, l_suppkey, l_extendedprice * (1.0 - l_discount) volume\n"
                "                    FROM part JOIN lineitem   ON p_partkey = l_partkey\n"
                "                    WHERE p_type = 'LARGE ANODIZED STEEL' \n"
                "                    ORDER BY l_orderkey, l_linenumber LIMIT $LIMIT),"
                " all_nations AS (\n"
                "    SELECT  o_orderyear, volume,  s.n_name as nation, nation_check\n"
                "    FROM   supplier_nation s,  lineitem_part l, "
                "       (SELECT * FROM  orders   ORDER BY o_orderyear, o_orderkey LIMIT $LIMIT) o,\n"
                "       customer_region  c\n"
                "    WHERE s.s_suppkey = l.l_suppkey AND l.l_orderkey = o.o_orderkey AND  o.o_custkey = c.c_custkey AND  o.o_orderdate >= date '1995-01-01' AND  o.o_orderdate < date '1996-12-31') \n"
                " SELECT  o_orderyear,\n"
                "  SUM(CASE WHEN  nation_check THEN volume ELSE 0 END)  / SUM(volume) as mkt_share\n"
                " FROM all_nations\n"
    ...
    std::map<int, std::string> aggregate_tpch_queries = {
            ...
            std::map<int, std::string> zk_tpch_queries = {
                ...
                {3, "SELECT o_orderkey,  SUM(l.l_extendedprice * (1 - l.l_discount)) as revenue, o_orderdate, o_shippriority "
                    " FROM (SELECT * FROM customer ORDER BY c_custkey LIMIT $LIMIT) c "
                    "     JOIN (SELECT * FROM orders ORDER BY o_orderkey LIMIT $LIMIT) o ON  c.c_custkey = o.o_custkey\n"
                    "     JOIN (SELECT * FROM  lineitem   ORDER BY l_orderkey, l_partkey, l_suppkey  LIMIT $LIMIT)  l ON l.l_orderkey = o.o_orderkey\n"
                    " WHERE  c.c_mktsegment = 'HOUSEHOLD'  AND o.o_orderdate < date '1995-03-25' AND l.l_shipdate > date '1995-03-25' "
                    " GROUP BY  o_orderkey, o.o_orderdate,  o.o_shippriority\n"
                    " ORDER BY  revenue DESC, o.o_orderdate \n"
                    "LIMIT 10"
                ...
                {8,     "SELECT   o_year, SUM(CASE WHEN  n_nation = 'KENYA' THEN volume ELSE 0.0 END) / SUM(VOLUME) AS mkt_share "
                        ...
                        " (SELECT * FROM  lineitem ORDER BY l_orderkey,  l_partkey, l_suppkey LIMIT $LIMIT)  l,"
                        "  (SELECT * FROM orders ORDER BY o_orderkey LIMIT $LIMIT) o, "
                        " (SELECT * FROM customer ORDER BY c_custkey LIMIT $LIMIT) c, "
                        " nation n1, nation n2, region r "
                        "       WHERE p.p_partkey = l.l_partkey AND s.s_suppkey = l.l_suppkey AND l.l_orderkey = o.o_orderkey AND  o.o_custkey = c.c_custkey AND c.c_nationkey = n1.n_nationkey AND n1.n_regionkey = r.r_regionkey AND r.r_name = 'AFRICA' AND s.s_nationkey = n2.n_nationkey "
                        "             AND o.o_orderdate BETWEEN DATE '1995-01-01' AND DATE '1996-12-31' AND  p.p_type = 'LARGE ANODIZED STEEL') AS all_nations "
                        "GROUP BY o_year "
                        "ORDER BY o_year"},
                {9, "WITH order_years AS (\n"

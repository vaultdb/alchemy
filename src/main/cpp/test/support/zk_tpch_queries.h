...
namespace vaultdb {
    ...
    std::map<int, std::string> zk_tpch_queries = {
            ...
            {3, "select\n"
                ...
                "  o.o_orderdate\n"
                " limit 10"},
            {5, "select n.n_name, sum(l.l_extendedprice * (100 - l.l_discount))::BIGINT as revenue\n"
   ...
   //             "  and o.o_orderdate >= date '1997-01-01'\n"
    //            "  and o.o_orderdate < date '1998-01-01'\n"
            ...
            {9, "WITH order_years AS (\n"
                ...
                "    	   n_name,\n"
                "     	    o_year,\n"
                "      	    l.l_extendedprice * (100 - l.l_discount) - ps.ps_supplycost * l.l_quantity as amount\n"
                "    	  from\n"
                "    	    yellow_parts p,\n"
                "      	    supplier s,\n"
                "      	    lineitem l,\n"
    ...
    std::map<int, std::string> truncated_zk_tpch_queries = {
            {1, "select \n"
                ...
                "  count(*)::BIGINT as count_order \n"
                "from \n"
                " (SELECT * FROM  lineitem ORDER BY (1), (2), (3), (5) LIMIT $LIMIT) t1 \n" // formerly  ORDER BY l_orderkey, l_linenumber
                " where \n"
                "  l_shipdate <= date '1998-08-03' \n"
                "group by \n"
                "  l_returnflag, \n"
            ...
            {3, "select\n"
                ...
                "  o.o_shippriority\n"
                " from\n"
                "   (SELECT * FROM  customer ORDER BY (1), (2), (3), (5) LIMIT $LIMIT)  c "
                "  JOIN  (SELECT * FROM  orders  ORDER BY (1), (2), (3), (5) LIMIT $LIMIT)  o ON  c.c_custkey = o.o_custkey\n"
                "     JOIN (SELECT * FROM  lineitem      ORDER BY (1), (2), (3), (5) LIMIT $LIMIT) l ON l.l_orderkey = o.o_orderkey\n"
                " where\n"
                "  c.c_mktsegment = 'HOUSEHOLD'\n"
                "  and o.o_orderdate < date '1995-03-25'\n"
                "  and l.l_shipdate > date '1995-03-25'\n"

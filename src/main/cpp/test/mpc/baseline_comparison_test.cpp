...
void
BaselineComparisonTest::runTest(const int &test_id, const string & test_name, const SortDefinition &expected_sort, const string &db_name, int type_num) {
    ...
    switch (type_num) {
        case 1:
            plan_file = Utilities::getCurrentWorkingDirectory() + "/conf/plans/experiment_1/baseline/baseline-" + test_name + ".json";
            break;
        case 2:
            plan_file = Utilities::getCurrentWorkingDirectory() + "/conf/plans/experiment_1/RewriteRules/mpc-" + test_name + ".json";
            break;
        case 3:
            plan_file = Utilities::getCurrentWorkingDirectory() + "/conf/plans/experiment_1/plaintext/plaintext-" + test_name + ".json";
            break;
    }
...
// ---------------------------------------------------------
// NOTE: SMCQL version
// Simple rewrite rules with NLJ(Not keyedjoin), SMA only
// no public tables duplicates, no complex calculation in local
// ---------------------------------------------------------
///////////////////////////////////////////////////////////////////////////////
...
TEST_F(BaselineComparisonTest, baseline_tpch_q3) {
    ...
    SortDefinition expected_sort{ColumnSort(-1, SortDirection::ASCENDING),
                                 ColumnSort(1, SortDirection::DESCENDING),
                                 ColumnSort(2, SortDirection::ASCENDING)};
    runTest(3, "q3", expected_sort, FLAGS_unioned_db, 3);
}


TEST_F(BaselineComparisonTest, baseline_tpch_q5) {
...
TEST_F(BaselineComparisonTest, baseline_tpch_q8) {
...
// *** Rewrite Rules Tests ***
// ---------------------------------------------------------
// NOTE: we have public tables duplicates, so possible for public join in local
// push down complex calculation to local
// ---------------------------------------------------------
///////////////////////////////////////////////////////////////////////////////

TEST_F(BaselineComparisonTest, rewrite_rules_tpch_q1) {
...
TEST_F(BaselineComparisonTest, rewrite_rules_tpch_q5) {
    //input_tuple_limit_ = 1000;
...
TEST_F(BaselineComparisonTest, rewrite_rules_tpch_q9) {
    // $0 ASC, $1 DESC
    SortDefinition  expected_sort{ColumnSort(0, SortDirection::ASCENDING), ColumnSort(1, SortDirection::DESCENDING)};

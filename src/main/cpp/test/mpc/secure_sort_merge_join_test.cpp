...
TEST_F(SecureSortMergeJoinTest, test_tpch_q3_lineitem_orders_customer) {
    ...
    // join output schema: (orders, customer)
    // o_orderkey, o_custkey, o_orderdate, o_shippriority, c_custkey
    GenericExpression<emp::Bit> *customer_orders_predicate = (GenericExpression<Bit> *) FieldUtilities::getEqualityPredicate<emp::Bit>(orders_input, 1,
                                                                                                                       customer_input, 4);
    ...
    cout << "CO output: " << *co_join->getOutput()->reveal() << endl;

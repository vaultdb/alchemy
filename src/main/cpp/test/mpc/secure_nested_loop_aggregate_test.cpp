...
void SecureNestedLoopyAggregateTest::runTest(const string &expected_sql,
    ...
    PlainTable *expected = EmpBaseTest::getExpectedOutput(expected_sql, 0);

    // run secure query
    SortDefinition sort_def = DataUtilities::getDefaultSortDefinition(1); // actually 2
    auto input = new SecureSqlInput(db_name_, query, false, storage_model_, sort_def, netio_, FLAGS_party);
...
void SecureNestedLoopyAggregateTest::runDummiesTest(const string &expected_sql,
                                                const vector<ScalarAggregateDefinition> &aggregators) const {
    ...
    auto input = new SecureSqlInput(db_name_, query, true, storage_model_, sort_def, netio_, FLAGS_party);
...
TEST_F(SecureNestedLoopyAggregateTest, test_count) {
    // set up expected output
...
TEST_F(SecureNestedLoopyAggregateTest, test_sum) {
...
TEST_F(SecureNestedLoopyAggregateTest, test_sum_dummies) {
...
TEST_F(SecureNestedLoopyAggregateTest, test_avg) {
...
TEST_F(SecureNestedLoopyAggregateTest, test_avg_dummies) {
...
TEST_F(SecureNestedLoopyAggregateTest, test_min) {
...
TEST_F(SecureNestedLoopyAggregateTest, test_min_dummies) {
...
TEST_F(SecureNestedLoopyAggregateTest, test_max) {
...
TEST_F(SecureNestedLoopyAggregateTest, test_max_dummies) {
    std::string query = "SELECT l_orderkey, l_linenumber,  l_shipinstruct <> 'NONE' AS dummy  FROM lineitem WHERE l_orderkey <=10 ORDER BY (1), (2)";
...
TEST_F(SecureNestedLoopyAggregateTest, test_tpch_q1_sums) {
...
TEST_F(SecureNestedLoopyAggregateTest, test_tpch_q1_avg_cnt) {
...
TEST_F(SecureNestedLoopyAggregateTest, tpch_q1) {

...
TEST_F(OMPCSortTest, tpchQ01Sort) {
    ...
    //string sql = "WITH input AS (" + limit_sql + ") SELECT * FROM input WHERE l_orderkey <= "   + std::to_string(FLAGS_cutoff) + " ORDER BY l_returnflag, l_linestatus";
    ...
    ExpressionMapBuilder<Bit> builder(packed_schema);
    ...
    if(FLAGS_validation) {
        ...
        cout << "observed: " << DataUtilities::printTable(observed) << endl;
        ...
        cout << "expected: " << DataUtilities::printTable(expected) << endl;

...
class OMPCKeyedSortMergeJoinTest : public OmpcBaseTest {
protected:
...
TEST_F(OMPCKeyedSortMergeJoinTest, test_tpch_q3_customer_orders) {
    ...
    // test fkey-pkey join
    KeyedSortMergeJoin<emp::Bit> *join = new KeyedSortMergeJoin(orders_project, customer_project, predicate);
...
TEST_F(OMPCKeyedSortMergeJoinTest, test_tpch_q3_lineitem_orders) {
// get inputs from local oblivious ops
    ...
    // test pkey-fkey join
    KeyedSortMergeJoin<emp::Bit> *join = new KeyedSortMergeJoin(lineitem_project, orders_project, predicate);
    join->setOperatorId(-2);
    SecureTable *join_res = join->run();
...
// compose C-O-L join
TEST_F(OMPCKeyedSortMergeJoinTest, test_tpch_q3_lineitem_orders_customer) {
    ...
    KeyedSortMergeJoin<Bit> *co_join = new KeyedSortMergeJoin(orders_project, customer_project, customer_orders_predicate);
    ...
    KeyedSortMergeJoin<Bit> *col_join = new KeyedSortMergeJoin(lineitem_project, co_join, lineitem_orders_predicate);

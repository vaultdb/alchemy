WITH cohort AS (SELECT pat_id, age_strata, sex, ethnicity, race, site_id, max(numerator::INT) numerator, max(denominator::INT) denominator, denom_excl
                FROM patient
		WHERE NOT denom_excl AND :selection
                GROUP BY pat_id, age_strata, sex, ethnicity, race, denom_excl, site_id
                ORDER BY pat_id),
     aggs AS (SELECT age_strata, sex, ethnicity, race, SUM(CASE WHEN numerator > 0  THEN 1 ELSE 0 END)::INT numerator_cnt,

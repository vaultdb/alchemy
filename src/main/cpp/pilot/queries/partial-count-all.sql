WITH single_site AS (
    ...
    WHERE NOT multisite AND NOT denom_excl),
     full_domain AS (
         ...
         FROM   (SELECT * FROM demographics_domain WHERE :selection)  d LEFT JOIN single_site p on d.age_strata = p.age_strata  AND d.sex = p.sex  AND d.ethnicity = p.ethnicity AND d.race = p.race)

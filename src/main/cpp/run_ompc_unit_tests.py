from ompc_runner import OmpcTestConfig
from ompc_runner import OmpcRunner
import sys
import argparse 

def run_ompc_tests(regex_=".*",label_="OmpcTest-",regex_exclude_="a^",label_exclude_="ZkTest",filter_="*",run_local_="True",is_host_="True",alice_host_="127.0.0.1", cutoff_=-2):
    config = OmpcTestConfig()
    ...
    for test_suite in config.test_suites:
        ...
        test_suite.edit_flag("unioned_db","tpch_unioned_150")
        test_suite.edit_flag("alice_db","tpch_alice_150")
        test_suite.edit_flag("bob_db","tpch_bob_150")
        test_suite.edit_flag("alice_host",alice_host_)
        if cutoff_ != -2:
            test_suite.edit_flag("cutoff",str(cutoff_))
    runner = OmpcRunner(config,is_host= (is_host_ == "True"))
    runner.run_local= (run_local_ == "True")
    runner.run_ompc_tests()
...
run_ompc_tests(regex_=args.regex, label_=args.label, regex_exclude_=args.regex_exclude, label_exclude_=args.label_exclude,filter_=args.filter,run_local_=args.run_local,is_host_=args.is_host,alice_host_=args.alice_host, cutoff_=args.cutoff)

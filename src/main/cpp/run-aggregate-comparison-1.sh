...
do
    ...
    $executable --party=$PARTY --cutoff=${cutoff_values[$i]} --dbname=${dbnames[$i]} --bitpacking="packed" >> "$log_file"

    echo "Running tests with Non Bit Packed" >> "$log_file"
    $executable --party=$PARTY --cutoff=${cutoff_values[$i]} --dbname=${dbnames[$i]} --bitpacking="non" >> "$log_file"
done

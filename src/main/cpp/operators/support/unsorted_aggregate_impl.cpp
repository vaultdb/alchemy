...
UnsortedAggregateImpl<B>::UnsortedAggregateImpl(const AggregateId &id, const FieldType &type,
    ...
    if(max_value > 0 && std::is_same_v<B, Bit>) {
        bit_packed_size_ =  (SystemConfiguration::getInstance().bitPackingEnabled()) ?  ceil(log2(static_cast<float>(max_value))) + 1 : TypeUtilities::getTypeSize(this->field_type_); // for sign bit

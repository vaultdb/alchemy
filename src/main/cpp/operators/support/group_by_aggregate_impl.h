...
public:
     GroupByCountImpl(const int32_t & ordinal, const FieldType & agg_type, const int & max_count = 0)  : GroupByAggregateImpl<Bit>(ordinal, agg_type)
    {
        ...
        int bit_cnt = (max_count == 0) ? 64 : static_cast<int>(ceil(log2(max_count)) + 1);
        if (!SystemConfiguration::getInstance().bitPackingEnabled()) bit_cnt = 64;
...
public:
    ...
    inline void accumulate(const PlainTable *table, const int & row,  const bool &group_by_match) override {
        int64_t incr = table->getDummyTag(row) ? 0L : 1L;
        if(!group_by_match)
            running_count_ = 0L;
...
template<typename B> class GroupByDistinctCountImpl;

    // GroupByDistinctCountImpl for secure (emp::Bit) version.
    template<>
    class GroupByDistinctCountImpl<emp::Bit> : public GroupByAggregateImpl<emp::Bit> {
    public:
        // Construct with the operand ordinal; optionally, you can provide an upper bound to compute the bit width.
        GroupByDistinctCountImpl(const int32_t & ordinal, const FieldType & agg_type, const int & max_count = 0)
                : GroupByAggregateImpl<emp::Bit>(ordinal, agg_type) {
            // For distinct count, we always return SECURE_LONG.
            GroupByAggregateImpl<emp::Bit>::aggregate_type_ = FieldType::SECURE_LONG;
            int bit_cnt = (max_count == 0) ? 64 : static_cast<int>(ceil(log2(max_count)) + 1);
            if (!SystemConfiguration::getInstance().bitPackingEnabled()) bit_cnt = 64;
            distinct_count_ = emp::Integer(bit_cnt, 0, PUBLIC);
            zero_i_ = emp::Integer(bit_cnt, 0, PUBLIC);
            one_i_ = emp::Integer(bit_cnt, 1, PUBLIC);
        }

        // Called on the first row of a group.
        inline void initialize(const SecureTable *table) override {
            emp::Bit dummy = table->getDummyTag(0);
            Field<emp::Bit> current = table->getField(0, this->aggregate_ordinal_);
            // For a non‐dummy row, set distinct_count_ = 1; otherwise 0.
            distinct_count_ = emp::If(dummy, zero_i_, one_i_);
            // Save the current value as the "last seen" value.
            last_value_ = current;
        }

        // Called for each subsequent row.
        inline void accumulate(const SecureTable *table, const int & row, const emp::Bit & group_by_match) override {
            emp::Bit dummy = table->getDummyTag(row);
            Field<emp::Bit> current = table->getField(row, this->aggregate_ordinal_);
            // For new group (group_by_match is false), reinitialize:
            // new_count = if(dummy) then 0 else 1.
            emp::Integer new_count = emp::If(group_by_match, distinct_count_,
                                             emp::If(dummy, zero_i_, one_i_));
            // For the same group, update only if the row is non-dummy and the current value differs from last_value_.
            emp::Bit is_diff = !(current == last_value_);
            emp::Bit should_increment = (!dummy) & is_diff;
            // Use secure conditional selection instead of a ternary operator.
            emp::Integer increment = emp::If(should_increment, one_i_, zero_i_);
            new_count = emp::If(group_by_match, distinct_count_ + increment, new_count);
            distinct_count_ = new_count;
            // Update last_value_ in the new group case.
            last_value_ = Field<emp::Bit>::If(!group_by_match, current, last_value_);
        }

        inline Field<emp::Bit> getResult() override {
            return SecureField(FieldType::SECURE_LONG, distinct_count_);
        }
        ...
        ~GroupByDistinctCountImpl() = default;

    private:
        emp::Integer distinct_count_;
        emp::Integer zero_i_;
        emp::Integer one_i_;
        Field<emp::Bit> last_value_;  // The last distinct operand value in the current group
    };

    // GroupByDistinctCountImpl for plain (bool) version.
    template<>
    class GroupByDistinctCountImpl<bool> : public GroupByAggregateImpl<bool> {
    public:
        // Construct with the operand ordinal (the field to count distinct)
        explicit GroupByDistinctCountImpl(const int32_t & ordinal, const FieldType & agg_type)
                : GroupByAggregateImpl<bool>(ordinal, agg_type), distinct_count_(0) {}

        // Called on the first row of a group.
        inline void initialize(const PlainTable *table) override {
            bool dummy = table->getDummyTag(0);
            Field<bool> current = table->getField(0, this->aggregate_ordinal_);
            if (!dummy) {
                last_value_ = current;
                distinct_count_ = 1;
            } else {
        ...
        // Called for each subsequent row.
        inline void accumulate(const PlainTable *table, const int & row, const bool & group_by_match) override {
            // If group_by_match is false, this row starts a new group.
            if (!group_by_match) {
                bool dummy = table->getDummyTag(row);
                Field<bool> current = table->getField(row, this->aggregate_ordinal_);
                if (!dummy) {
                    last_value_ = current;
                    distinct_count_ = 1;
                } else {
                ...
                // Same group: update only if the row is non‐dummy and its value differs from the last seen.
                bool dummy = table->getDummyTag(row);
                Field<bool> current = table->getField(row, this->aggregate_ordinal_);
                if (!dummy && !(current == last_value_)) {
                    distinct_count_ += 1;
                    last_value_ = current;
                }
            }
        ...
        inline PlainField getResult() override {
            return PlainField(FieldType::LONG, distinct_count_);
        }

        inline FieldType getType() const override {
            return FieldType::LONG;  // count returns a LONG
        }
    ...
    private:
        ...
        Field<bool> last_value_;  // Holds the last operand value seen in the current group
    };

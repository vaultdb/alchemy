...
void KeyedSortMergeJoin<B>::setup() {
    ...
    // *** begin debug expression setup ***/
    // auto read_field = new InputReference<B>(0, QueryFieldDesc(0, "key", "", int_field_type_ ));
    // auto constant_input = (std::is_same_v<B, Bit>) ? new LiteralNode<B>(Field<B>(int_field_type_, Integer(32, debug_key_)))
    //                             : new LiteralNode<B>(Field<B>(int_field_type_, debug_key_));
    // auto equality_check = new EqualNode<B>( read_field,  constant_input);
    //
    // debug_suspect_ = new GenericExpression<B> (equality_check, "predicate", bool_field_type_);

    // *** end debug expression setup ***/
...
QueryTable<B> *KeyedSortMergeJoin<B>::runSelf() {
    ...
    // cout << "Augmented lhs: \n" << DataUtilities::printFilteredTable(augmented.first, debug_suspect_, -1, true) << "\n";
    // cout << "Augmented rhs: \n" << DataUtilities::printFilteredTable(augmented.second, debug_suspect_, -1, true) << "\n";

    QueryTable<B> *expanded_lhs, *expanded_rhs;

    expanded_lhs = expand(augmented.first);
    // cout << "Expanded lhs: \n" << DataUtilities::printFilteredTable(expanded_lhs, debug_suspect_, -1, true) << "\n";

    expanded_rhs = expand(augmented.second);

    // cout << "Expanded rhs: \n" << DataUtilities::printFilteredTable(expanded_rhs, debug_suspect_, -1, true) << "\n";
...
pair<QueryTable<B> *, QueryTable<B> *>  KeyedSortMergeJoin<B>::augmentTables(QueryTable<B> *lhs, QueryTable<B> *rhs) {
    ...
    // cout << "LHS projected:\n " << DataUtilities::printFilteredTable(lhs_prime_, debug_suspect_, -1, true) <<  '\n';
    // cout << "RHS projected:\n " << DataUtilities::printFilteredTable(rhs_prime_, debug_suspect_, -1, true) <<  '\n';
    ...
    // cout << "Unioned and sorted: \n" << DataUtilities::printFilteredTable(sorted, debug_suspect_, -1, true) << '\n';

    initializeAlphas(sorted);

    // cout << "Initialized alphas: \n" << DataUtilities::printFilteredTable(sorted, debug_suspect_, -1, true) << '\n';
    ...
    PageId lhs_pid{output.first->table_id_, 0, 0};
    auto lhs_start = SystemConfiguration::getInstance().bpm_.getPagePtr<B>(lhs_pid);
...
QueryTable<B> *KeyedSortMergeJoin<B>::unionTables(QueryTable<B> *lhs, QueryTable<B> *rhs, const QuerySchema & dst_schema) {
    ...
    int cursor = 0;
    int row_size_bytes = dst_schema.rowSizeBytes();
    ...
    for(int i = lhs->tuple_cnt_ - 1; i >= 0; --i) {
        auto r = lhs->serializeRow(i, row_size_bytes);
        unioned->deserializeRow(cursor, r);
        ++cursor;
    }

    for(int i = 0; i < rhs->tuple_cnt_; ++i) {
        auto r = rhs->serializeRow(i, row_size_bytes);
        unioned->deserializeRow(cursor, r);
        ++cursor;
    }
...
QueryTable<bool> *KeyedSortMergeJoin<bool>::revertProjection(QueryTable<bool> *src, const map<int, int> &expr_map,
    ...
    QueryTable<bool> *to_project = nullptr;
    ...
    if(lhs_smaller_ == is_lhs) {
        ...
        for(int i = 0; i < tuple_cnt; ++i) {
            auto r = src->serializeRow(i, synthetic_schema.rowSizeBytes());

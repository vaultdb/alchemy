...
: sort_definition_(sorted_on), system_conf_(SystemConfiguration::getInstance()) {
    ...
    operator_id_ = system_conf_.operator_cnt_++;
...
QueryTable<B> *Operator<B>::run() {
    ...
    if (lhs_child_) lhs_child_->unpin();  // resets only when pin_count_ == 0
    if (rhs_child_) rhs_child_->unpin();
...
Operator<B>* Operator<B>::canonicalizedHelper(std::unordered_map<int, Operator<B>*>& canonicalMap) {
    ...
    if (canonicalMap.find(id) != canonicalMap.end()) {
        canonicalMap[id]->pin();  // Another parent is using this node

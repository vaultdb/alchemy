...
QueryTable<B> *BasicJoin<B>::runSelf() {
    ...
    for(int i = 0; i < lhs->tuple_cnt_; ++i) {
        ...
        for(int j = 0; j < rhs->tuple_cnt_; ++j) {
            if (!isCartesian_) {
                // Evaluate the join predicate normally.
                B selected = Join<B>::predicate_->call(lhs, i, rhs, j).template getValue<B>();
                B dst_dummy_tag = (!selected) | lhs_dummy_tag | rhs->getDummyTag(j);
                this->output_->setDummyTag(cursor, dst_dummy_tag);
            } else {
                // For a cartesian join, simply ignore the predicate and combine dummy tags.
                this->output_->setDummyTag(cursor, lhs_dummy_tag | rhs->getDummyTag(j));
            }
            ++cursor;
        }
    }

...
template<typename B>
LeftKeyedJoin<B>::LeftKeyedJoin(Operator<B> * lhs, Operator<B> * rhs, const int & fkey, Expression<B> *predicate, const SortDefinition & sort)
...
template<typename B>
LeftKeyedJoin<B>::LeftKeyedJoin(QueryTable<B> *lhs, QueryTable<B> *rhs, const int & fkey, Expression<B> *predicate, const SortDefinition & sort)
        : Join<B>(lhs, rhs, predicate, sort), foreign_key_input_(fkey) {
    assert(fkey == 0 || fkey == 1);
...
template<typename B>
QueryTable<B> *LeftKeyedJoin<B>::foreignKeyPrimaryKeyJoin() {
...
template<typename B>
QueryTable<B> *LeftKeyedJoin<B>::primaryKeyForeignKeyJoin() {
    ...
    for(uint32_t i = 0; i < rhs_table->tuple_cnt_; ++i) {
        ...
        dst_dummy_tag = rhs_dummy_tag; // carry over previous dummy tag for left-join simulation

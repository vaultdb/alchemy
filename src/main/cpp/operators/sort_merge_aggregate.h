...
public:
    ...
    void updateCollation() override {
        ...
        assert(GroupByAggregate<B>::sortCompatible(child_sort, this->group_by_, this->effective_sort_));


        SortDefinition  output_sort_def;
        // // map sort order to that of child
        // if (!this->effective_sort_.empty()) {
        //     for (int i = 0; i < this->effective_sort_.size(); ++i) {
        //         output_sort_def.emplace_back(this->effective_sort_[i]);
        //     }
        // }
        // else {
            for(size_t idx = 0; idx < this->group_by_.size(); ++idx) {
                // projecting the attribute in group_by_[idx] to the ith position in output
                // JMR: if this fails, that means your inputs are probably not collated correctly by the group-by cols!

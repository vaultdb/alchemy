...
QueryTable<B> *Sort<B>::runSelf() {
    ...
    auto child_collation = child->getSortOrder();
    if (child_collation.size() >= this->sort_definition_.size() && std::equal(this->sort_definition_.begin(), this->sort_definition_.end(), child_collation.begin())) {
        return this->output_;
    }
...
QueryTable<B> *Sort<B>::normalizeTable(QueryTable<B> *src) {
    ...
    map<int, int> old_to_new_pos;  // Maps original position to new position
    SortDefinition new_sort_definition(this->sort_definition_.size());

    // First pass: move sort keys to front (keeping original code)
    for(int i = this->sort_definition_.size()-1; i >= 0; --i) {
        ...
        if (key.first != -1) {
            ...
            old_to_new_pos[key.first] = write_cursor;  // Store the mapping
        }
        sort_key_size_bits_ += src->getSchema().getField(key.first).size();
        ++write_cursor;
    }

    // Map remaining non-sort columns
    ...
    // Create new sort definition based on the original order but using new positions
    for(size_t i = 0; i < this->sort_definition_.size(); ++i) {
        auto old_key = this->sort_definition_[i];
        if(old_key.first != -1) {
            new_sort_definition[i] = {old_to_new_pos[old_key.first], old_key.second};
        }
    }
    ...
    dst->order_by_ = new_sort_definition;
    return dst;
}

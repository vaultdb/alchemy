...
size_t OperatorCostModel::basicJoinCost(const BasicJoin<Bit> *join) {
    ...
    size_t per_row_cost = 0;

    if (predicate != nullptr) {
        // We assume that a non-null predicate is generic.
        assert(predicate->exprClass() == ExpressionClass::GENERIC);
        ExpressionNode<Bit> *root = ((GenericExpression<Bit> *) predicate)->root_;
        ExpressionCostModel<Bit> cost_model(root, join->getOutputSchema());
        per_row_cost = cost_model.cost() + 3; // +3 for bookkeeping on dummy tag
    } else {
        // For a cartesian join (predicate is nullptr), assume only the bookkeeping cost.
        per_row_cost = 3;
    }
...
size_t OperatorCostModel::sortCost(const Sort<Bit> *sort) {
    // if it's already sorted correctly, we skip the sort
    // mirrored logic in Sort::runSelf()
    auto child = sort->getChild();
    auto child_collation = child->getSortOrder();
    if (child_collation.size() >= sort->getSortOrder().size() &&
        std::equal(sort->getSortOrder().begin(), sort->getSortOrder().end(), child_collation.begin())) {
        return 0;
    }

    //  need child (as opposed to taking output card) in case of truncation
    size_t n = sort->getChild()->getOutputCardinality();

...
void PlanOptimizer<B>::recurseJoin(Operator<B> *join) {
    ...
    // Unwrap dummy -> PROJECT pattern
    if (rhs->getOperatorId() == -1 && rhs->getType() != OperatorType::PROJECT) {
        if (rhs->getChild()->getType() == OperatorType::PROJECT) {
            candidate = rhs->getChild();  // candidate is now the PROJECT node
        }
    }

    // Now check if candidate is a PROJECT and its child is a JOIN
    if (candidate->getType() == OperatorType::PROJECT) {
        auto child = candidate->getChild();
        if (child->getType() == OperatorType::KEYED_NESTED_LOOP_JOIN ||
            child->getType() == OperatorType::KEYED_SORT_MERGE_JOIN) {

            auto right_join = child;
        ...
        // Unwrap rhs if it's a SORT operator

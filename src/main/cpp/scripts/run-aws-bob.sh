...
run_test() {
  ...
  sudo ./bin/cost_optimized_test --party=$party --alice_host=126.137.1.10 --unioned_db=$unioned_db --alice_db=$alice_db --bob_db=$bob_db --filter="$test_name" 2>&1 | tee -a "$log_file"

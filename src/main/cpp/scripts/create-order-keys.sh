...
psql tpch_unioned_1500 -c "DROP TABLE IF EXISTS order_keys"
psql tpch_unioned_1500 -c "SELECT o_orderkey INTO order_keys FROM orders"
echo "DROP TABLE IF EXISTS order_keys;" > order_keys.sql
pg_dump tpch_unioned_1500 -t order_keys >> order_keys.sql
psql tpch_alice_1500 < order_keys.sql
psql tpch_bob_1500 < order_keys.sql

...
echo "Running cloud setting tests for Alice (Party $PARTY)"

for ((i=0; i<${#unioned_dbs[@]}; i++)); do
    for test_name in "${filters[@]}"; do
        LOG_FILE="$LOG_DIR/alice_${test_name}_${unioned_dbs[$i]}.log"
        echo "Running Alice test on DB: ${unioned_dbs[$i]}, Filter: ${test_name}"

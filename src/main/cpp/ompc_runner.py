...
class OmpcTestSuite:
    ...
    def add_flag(self,name,value,enabled):
        flag=OmpcFlag(name,value,enabled)
    ...
    def add_from_ctest(self,regex="",label="",regex_exclude="",label_exclude=""):
        ...
        for test in tests_json:
            ...
            if not prev_suite_name == suite_name:
                test_suite = OmpcTestSuite(suite_name,test["command"][0])
...
class OmpcRunner:
    ...
    def run_ompc_test_locally(self,test_suite):
        print("running ompc test suite " + test_suite.name + " locally ")

        log_level = (test_suite.find_flag_by_name("log_level")).value
 
        test_suite.edit_flag("log_level", "6")
        alice_flag=OmpcFlag("party","1",True)
        ...
        bob_flag=OmpcFlag("party","2",True)
        test_suite.flags.append(bob_flag)
        self.fork_background_test_process(test_suite)
        test_suite.flags.pop()

        test_suite.edit_flag("log_level",log_level)
        carol_flag=OmpcFlag("party","3",True)
        test_suite.flags.append(carol_flag)
        self.fork_background_test_process(test_suite)
        test_suite.flags.pop()

        test_suite.edit_flag("log_level",log_level)
        tp_flag=OmpcFlag("party","10086",True)
        test_suite.flags.append(tp_flag)
        self.fork_test_process(test_suite)
        test_suite.flags.pop()

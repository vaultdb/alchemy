...
Value::Value(const Value &val) { setValue(val); }

Value::Value(const int64_t &val) { setValue(val); }

Value::Value(const int32_t &val) { setValue(val); }
Value::Value(const bool &val) { setValue(val); }

Value::Value(const emp::Bit &val) { setValue(val); }

Value::Value(const emp::Float &aFloat) { setValue(aFloat); }

Value::Value(TypeId type, const emp::Integer &val) { setValue(type, val); }

Value::Value(const float &val) { setValue(val); }

Value::Value(const string &val) { setValue(val); }
...
int32_t Value::getInt32() const {
  if (value_.unencrypted_val.which() ==
      1) { // check that this converts to a int32_t
...
bool Value::getBool() const {
  if (value_.unencrypted_val.which() ==
      0) { // check that this converts to a int32_t
...
int64_t Value::getInt64() const {
  if (value_.unencrypted_val.which() ==
      2) { // check that this converts to a int32_t
...
float_t Value::getFloat32() const {
  if (value_.unencrypted_val.which() ==
      3) { // check that this converts to a int32_t
...
std::string Value::getVarchar() const {
  if (value_.unencrypted_val.which() ==
      5) { // check that this converts to a int32_t
    return boost::get<std::string>(value_.unencrypted_val);
  }
}

emp::Integer Value::getEmpInt() const {
  assert(value_.emp_integer_.get() !=
         nullptr); // do not return uninitialized vals
...
emp::Float Value::getEmpFloat32() const {
  assert(value_.emp_float32_.get() !=
         nullptr); // do not return uninitialized vals
...
void Value::setValue(const std::string &aString) {
  ...
  is_encrypted_ = false;
  value_.unencrypted_val =
      aString; // = ValueStruct(std::make_unique<std::string>(aString));
}
...
//    std::ostream &vaultdb::types::operator<<(std::ostream &strm, const Value
//    &aValue) {
...
string Value::getValueString() const {
  ...
  case TypeId::ENCRYPTED_INTEGER32: {
    ...
    // #else
    // int32_t decrypted = .getEmpInt()->reveal<int32_t>((int)
    // EmpParty::PUBLIC); return std::to_string(decrypted);
    // #endif
  }
  case TypeId::ENCRYPTED_INTEGER64: {
  ...
  case TypeId::ENCRYPTED_BOOLEAN: {
    // bool decrypted = getEmpBit()->reveal<bool>((int) EmpParty::PUBLIC); //
    // returns a bool for both XOR and PUBLIC return  decrypted ? "true" :
    // "false";
...
void Value::setType(TypeId type) { type_ = type; }
...
Value Value::reveal(const int &empParty) const {
  ...
  case types::TypeId::ENCRYPTED_BOOLEAN: {
    bool decrypted = this->getEmpBit().reveal<bool>(
        (int)empParty); // returns a bool for both XOR and PUBLIC
  ...
  case types::TypeId::ENCRYPTED_VARCHAR: {
    ...
    emp::ProtocolExecution::prot_exec->reveal(
        bools, (int)empParty, (emp::block *)&encryptedString.bits[0], bitCount);
